...
function save_so($db,$user_id,$ongkos,$jasa_kirim,$sold_to_customer,$cid,
	...
	if(CONVERT_SO_INVOICE){
		save_invoice($sales_order_number);
	}
...
function update_so($db,$user_id,$ongkos,$jasa_kirim,$sold_to_customer,$cid,
    ...
    status=1,amount=0,salesman='$user_id',freight='$ongkos',shipped_via='$jasa_kirim',cid='$cid',
    type_of_invoice='Simple',source_of_order='maxoneshop',ref1='$table_no',
	ref2='$terminal',ref3=null 
	where sales_order_number='$sales_order_number'";
    //echo $sql;
	//ref3 status autoprint simak so dibuat null lagi biar bisa cetak ulang
	...
	if(CONVERT_SO_INVOICE){
		update_invoice($sales_order_number);
	}
...
function save_item($db,$sales_order_number,$item_no,$qty,$price,$amount,$agent_id,
	...
	//if($amount=="")$amount="0";
	//if($amount=="0"){
		//$amount=($qty*$price)-(($qty*$price)/$disc);
	///}
	
	if($line_number==""||$line_number=="0"||$line_number=="null"||){
		$line_number=0;
	}
    if($line_number==0){
    ...
    mysqli_query($db->con,$s);
	if($line_number==0){
		$line_number=mysqli_insert_id($db->con);
	}
	if($salesman!=""){
...
function save_invoice($sales_order_number){
	$s="insert into invoice(invoice_number,invoice_type,type_of_invoice,
	sold_to_customer,ship_to_customer,
	invoice_date,salesman,discount,disc_amount_1,amount)
	select sales_order_number,'I','Simple',sold_to_customer,ship_to_customer,
	sales_date,salesman,discount,disc_amount_1,amount from sales_order 
	where sales_order_number='$sales_order_number'";
	mysqli_query($s);
}
function update_invoice($sales_order_number){
	$s="update invoice inner join sales_order so 
		on so.sales_order_number=invoice.invoice_number 
		set invoice.sold_to_customer=so.sold_to_customer,
		invoice.amount=so.amount,invoice.discount=so.discount,
		invoice.discount_amount=so.discount_amount
		where invoice.invoice_number='$sales_order_number'";
	mysqli_query($s);
}
function save_invoice_item($line_number){
	$line_faktur=0;
	$s="select line_number from invoice_lineitems 
	where from_line_number='$line_number'";
	if($q=mysqli_query($db->con,$s)){
		if($r=mysqli_fetch_object($q)){
			$line_faktur=$r->line_number;
		}
	}
	if($line_faktur==0){
		$s="insert into invoice_lineitems(invoice_number,item_number,
		description,quantity,unit,price,discount,
		discount_amount,amount,mu_qty,mu_harga,multi_unit,
		warehouse_code,cost,from_line_number)
		select sales_order_number,item_number,description,quantity,unit,price,discount,
		discount_amount,amount,mu_qty,mu_harga,multi_unit,warehouse_code,cost 
		from sales_order_lineitems where line_number='$line_number'";
	} else {
		$s="update invoice_lineitems i inner join sales_order_lineitems sol 
		on sol.line_number=i.from_line_number 
		set i.quantity=sol.quantity,
		i.item_number=sol.invoice_number,
		i.description=sol.description,
		i.price=sol.price,
		i.discount=sol.discount,
		i.disc_amount_1=sol.disc_amount_1,
		i.amount=sol.amount,
		i.mu_qty=sol.mu_qty,
		i.mu_harga=sol.mu_harga,
		i.multi_unit=sol.multi_unit,
		i.unit=sol.unit,
		i.cost=sol.cost, 
		i.warehouse_code=sol.warehouse_code 
		where invoice_lineitems.line_number='$line_faktur'";
	}
	mysqli_query($db->con,$s);
