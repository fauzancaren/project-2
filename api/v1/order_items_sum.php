...
$dataset=null;
$sql ="select si.item_number,si.description,sum(si.quantity) as quantity,
sum(si.amount) as amount
from sales_order_lineitems si left join sales_order s on s.sales_order_number=si.sales_order_number
...
if($order_ready_delivered!="" && $api_cmd=="new"){
	$sql.=" and si.update_status='$order_ready_delivered'";
	if($merchant!=""){
		$sql.=" and si.warehouse_code='$merchant'";
	}	
} else  if ($api_cmd=="history_kurir") {
	$sql.=" and si.status='8'";
} else {
...
$sql.=" group by si.item_number,si.description";
