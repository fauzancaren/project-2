...
$dataset=null;
$sql ="select s.supplier_number,s.supplier_name,sum(sol.amount) as amount 
from sales_order_lineitems sol left join sales_order so
on so.sales_order_number=sol.sales_order_number
left join inventory i on i.item_number=sol.item_number  
left join suppliers s on s.supplier_number=i.supplier_number 
where 1=1";

if($order_ready_delivered!="" && $api_cmd=="new"){
	$sql.=" and so.update_status='$order_ready_delivered'";
	if($merchant!=""){
		$sql.=" and so.warehouse_code='$merchant'";
	}	
} else  if ($api_cmd=="history_kurir") {
	$sql.=" and so.status='8'";
} else {
	if($status!="") {
		...
		$sql.=" and so.status not in ('2','6','8')";
	}	
}
if($search!=""){
...
if($d1!=""){
	$sql.=" and so.sales_date between '$d1' and '$d2' ";
}
$sql.=" group by s.supplier_number,s.supplier_name";
