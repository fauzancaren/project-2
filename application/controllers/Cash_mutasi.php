...
class Cash_mutasi extends CI_Controller {
	...
	function __construct()
	{
		...
		$this->sql="select voucher,check_date,payment_amount,posted,
			account_number,payee,supplier_number,ref1,
			trans_type,check_number,memo,trans_id,org_id
...
//            $data['account_number_list']=$this->bank_accounts_model->account_number_list();
	...
	function save(){
		...
		$id=$this->check_writer_model->save_trx($data);
	...
	function update()
	{
		...
		//$message=show_error("cash_mutasi.save()");
		if($message=="") $message='update success';
		$this->view($data['voucher'],$message);
	}
	...
	function view($id,$message=null){
		$id=urldecode($id);
		 ...
		 if($data['payment_amount']==0){
			 $data['payment_amount']=$data['deposit_amount'];
		 }
		 $data['mode']='view';
