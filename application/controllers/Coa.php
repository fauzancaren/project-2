...
class Coa extends CI_Controller {
	...
	function __construct()
	{
        ...
        $this->sql="select account,account_description,db_or_cr,
    ...
    function browse_data($offset=0,$limit=20,$nama=''){
        ...
        $search=$this->input->get("sid_nama");
		...
		if($search!='')$sql.=" and (account='$search' or account_description like '$search%')";
    ...
    function save(){
        ...
        $data["message"]=$success?"Data sudah disimpan":show_error("coa.save");
        echo json_encode($data);
    ...
    function save_group(){
        ...
        $data["success"]=$success;
        $data["message"]=$success?"Data sudah disimpan":show_error('coa.save_group');
        echo json_encode($data);
    }
...
function import_excel(){
    $p=$this->input->post();
        ...
        if($_FILES["file_excel"]["size"] > 0)
        {
            ...
            {
                echo "Error: Row $i : ".$kode." : ".show_error('import_excel')."</br>";
