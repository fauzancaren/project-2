...
class Company extends CI_Controller {
    ...
    function __construct()    {
        ...
        $this->sql="select company_code,company_name,street,city_state_zip_code,phone_number,
        	fax_number,email
            from preferences";
   ...
   function browse($offset=0,$limit=50,$order_column='sales_order_number',$order_type='asc'){
	...
	function gl_link(){
		if($this->input->post()){
			...
			if($this->input->post('txtCoaStockWip')!=""){
				$this->sysvar->getvar("COA Stock WIP",$this->input->post('txtCoaStockWip'));
			}            
			$this->sysvar->save('COA Stock WIP',$this->acc_id($this->input->post('txtCoaStockWip')));
	...
	function others($cmd=""){
        $sql="select varname,varvalue,keterangan,category,section,id 
            from system_variables where section='others'";
        if($data=$this->input->post()){
            while ($varvalue = current($data)) {
                $varname=key($data);
                next($data);
   ...
   function execute_sql_file($file_sql,$db)
   {
	   ...
	   $err=array();
		...
		foreach($sqls as $statement){
			...
			if(!$ok){
				$err[]=show_error('execute_sql_file');
			}
		}
		//$this->sql_error=$err;
		return $ok;
   ...
   function toko_add(){
		...
		if ($this->db->insert('toko_master',$data)){
            ...
            $gdg["location_number"]=$data['code'];
            $gdg["company_name"]=$data['code_company'];
            $gdg["address_type"]='Penyimpanan';
            $gdg["street"]=$data['address'];
            $gdg["attention_name"]=$data['toko_name'];
            $gdg["phone"]=$data['phone'];
            $this->shipping_locations_model->save($gdg);
   ...
   function toko($cmd="",$offset=0,$limit=50,$company_code=""){
        ...
        if($cmd=="delete"){
            ...
            $msg="";
            $gdg="";
            $gdg=$this->db->where("id",$id)->get("toko_master")->row()->code;
            $success=$this->db->where("id",$id)->delete("toko_master");
            if(!$success)$msg='Some errors occured.';
            
            $this->load->model("shipping_locations_model");
            $this->shipping_locations_model->delete($gdg);
    ...
    function check_database($password)  {
       ...
       if($result)  {
         ...
         foreach($result->result() as $row) {
            ...
            //manual kan dulu karena banyak yg belum serius cek finansial periode
            $min_date="2018-01-01";
            $max_date="2028-12-01";
    ...
    function warehouse($company_code){
        $s="select location_number,company_name 
        from $company_code.shipping_locations order by location_number";
        $data=null;
