...
class Customer_type extends CI_Controller {
	...
	function save(){
		...
		if($ok){
			...
			echo json_encode(array("msg"=>"Error ".show_error('customer_type.save')));
		}
	}	
   ...
   function browse($offset=0,$limit=50,$order_column="",$order_type='asc'){
	...
	function find($nomor){
		...
		$query=$this->db->query("select * from $this->table_name where type_name='$nomor'");
	...
	function save_item_price()
	{
		...
		if($id=="")$id=0;
		if($data['disc_prc_from']=='')unset($data['disc_prc_from']);
		if($data['disc_prc_to']=='')unset($data['disc_prc_to']);
