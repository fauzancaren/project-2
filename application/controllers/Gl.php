...
class Gl extends CI_Controller {
    ...
    function rpt($id){
    	 switch ($id) {
			...
			case 'jurnal':
			 ...
			 case 'neraca_saldo' or 'rugi_laba':
				 $data['date_from']=date('Y-m-d 00:00:00');
				 $data['date_to']=date('Y-m-d 23:59:59');
				 $data['select_date']=false;
                 $data['criteria1']=true;
                 ...
                 $data['text1']=date("Y-m");
                 $data['criteria2']=true;
                 $data['label2']='Tampil Saldo Nol (0-Ya, 1-Tidak)';
                 $data['text2']='1';
	...
	function view_doc($nomor){
		$jenis="";
		$s="select * from q_all_trans where nomor_bukti='$nomor'";
		...
		echo "<p>Nomor: $nomor </p><p>Jenis: $jenis </p>";
		$url="";
		switch ($jenis) {
			case 'kas masuk':
				$url=base_url()."/index.php/cash_in/view/".$nomor;
				break;
			case 'kas keluar':
				$url=base_url()."/index.php/cash_out/view/".$nomor;
				break;
				
			default:
				# code...
				echo "<p style='color:red'>Undefined: $jenis</p>";
				break;
		}
		if($url!=""){
				# code...
				echo "<p><b>Loading....</b></p>
				<script>
