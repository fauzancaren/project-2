...
//		$data['supplier_list']=$this->supplier_model->select_list();
		...
		if($data['taxable']=="") $data['taxable']='1';
        ...
        if($data['cost_method']=="")$data['cost_method']=0;		
        ...
        $setclass['dlgBindId']="inventory_class";
        $setclass['dlgRetFunc']="$('#class').val(row.kode);";
        $setclass['dlgCols']=array( 
                    array("fieldname"=>"kode","caption"=>"Kode","width"=>"180px"),
                    array("fieldname"=>"class","caption"=>"Nama Class","width"=>"80px")
                );          
        $data['lookup_class']=$this->list_of_values->render($setclass);
		$this->ShowItemCost=allow_mod("ShowItemCost");
		$data['ShowItemCost']=$this->ShowItemCost;
...
///			$this->template->display_browse2($data);
	...
	function browse($offset=0,$limit=10,$order_column='company',$order_type='asc')
	{
		...
		if(!$query_proc || $this->input->get('query_proc')=='0'){
			$data['caption']='DAFTAR BARANG DAN JASA';
			$data['controller']='inventory';		
			$aCaption=array('Kode','Nama Barang','Qty','Unit',"Qty2","Unit2",'Harga Jual',
				'Kode Supplier','Nama Supplier','Kelas','Category','Cat Name','Sub','Sistim',
				'Kode Lama','CID');
			//'Harga Beli',
				$aFields = array('item_number','description','quantity_in_stock','unit_of_measure','','','retail','supplier_number','supplier_name','class','category','cat_name','sub_category','type_of_invoice',
					'kode_lama','cid');
			//'cost_from_mfg',

			$this->ShowItemCost=allow_mod("ShowItemCost");
			if ($this->ShowItemCost) {
				unset($aCaption['Harga Beli']);
				unset($aFields['cost_from_mfg']);

			}
		
			$data['fields_caption']=$aCaption;
			$data['fields']=$aFields;
	...
	function find($item_number='',$cust_type='',$min_qty=0){
		...
		if($data){
			...
			$disc_prc=$this->promo_disc_item($item_number,$cust_no);

			$data['disc_prc_1']=$disc_prc['disc_prc_1'];
			$data['disc_prc_2']=$disc_prc['disc_prc_2'];
			$data['disc_prc_3']=$disc_prc['disc_prc_3'];
			if(isset($disc_prc['promosi_code']))$data['promosi_code']=$disc_prc['promosi_code'];
			if(isset($disc_prc['tipe']))$data['tipe']=$disc_prc['tipe'];
		...
		$qty_gudang=null;
		$s="select warehouse_code,quantity from inventory_warehouse 
		where item_number='$item_number' 
		order by warehouse_code";
		if($q=$this->db->query($s)){
			$i=0;
			foreach($q->result() as $rqty){
				$qty_gudang[$i]['warehouse_code']=$rqty->warehouse_code;
				$qty_gudang[$i]['quantity']=$rqty->quantity;
				$i++;
			}
		}
...
/* create_graph($konfigurasi_grafik, $data, $tipe_grafik, $judul_pd_grafik, $nama_berkas) */		
   ...
   function qty_gudang2($item_number){
	...
	function kartu_stock($item_number)
	{
        ...
        $this->qry_kartu_stock_union_model->_output_tmp=true;
        $sql=$this->qry_kartu_stock_union_model->GetQtyStockGdgSql($item_number,$gudang,"",$date_from);

        $sql="select sum(qty) as saldo  from tmp_kartu_stock_1 ";
		...
		$rows[0]=array("no_sumber"=>"SALDO","tanggal"=>"SALDO","jenis"=>"SALDO","qty_masuk"=>number_format(0,2),
		"qty_keluar"=>number_format(0,2),
			"saldo"=>number_format($awal,2),"gudang"=>"");
		...
		$this->qry_kartu_stock_union_model->_output_tmp=true;
        $s = $this->qry_kartu_stock_union_model->getSql("list",$date_from, $date_to, $item_number, $gudang);

		if($gudang!="")$sql.=" and gudang='$gudang'";
		$sql.=" order by tanggal";

		$s="select txndate as tanggal,document as no_sumber,qtyin as qty_masuk,
		qtyout as qty_keluar,qty as saldo,doctype as jenis,gudang
		 from tmp_kartu_stock order by txndate";
	...
	function promo_disc_item($item_number,$cust_no){
		$disc1=0; $disc2=0; $disc3=0;
		$ptype=0; $prcode=""; $flag1=0; $flag2=0; $flag3=0;
		//1. cari discount all item
		...
		//2a. cari dari diskon allitem (flag1=1) dan allcustomer flag2=1
		$foundOnDiscMember=false;
		$disc1m=0;	$disc2m=0;	$disc3m=0;
		if($disc1==0 && strtolower($cust_no)!="cash"){
			$s="select p.nilai,p.nilai2,p.nilai3,p.flag1,p.flag2,p.flag3,p.promosi_code,p.tipe
			from promosi_item i 
			join promosi_disc p on p.promosi_code=i.promosi_code where '".date('Y-m-d H:i:s')."' 
			between p.date_from and p.date_to  and p.category='1' and p.tipe='2' 
			and p.flag1='1' and p.flag2='1'  ";	
			if($q=$this->db->query($s)){
				if($r=$q->row()){
					$disc1m=$r->nilai;
					$disc2m=$r->nilai2;
					$disc3m=$r->nilai3;
					$foundOnDiscMember=true;
					$ptype=$r->tipe;
					$prcode=$r->promosi_code;
				}
			}
		}
		//3. gak ketemu? cari di dikson promo
		//|| $foundOnDiscMember
		if($disc1==0 ){
			$s="select p.nilai,p.nilai2,p.nilai3,p.flag1,p.flag2,p.flag3,p.promosi_code,p.tipe 
			from promosi_item i 
			join promosi_disc p on p.promosi_code=i.promosi_code where '".date('Y-m-d H:i:s')."' 
			between p.date_from and p.date_to  and p.category='1' and (p.tipe='2' or p.tipe='1') and p.flag1='0' 
		...
		//4. cari dari category,supplier, merk
		$category="";$supplier="";$manufacturer="";
		if($disc1==0){
			$s="select category,supplier_number,manufacturer from inventory 
			where item_number='$item_number'";
			if($q=$this->db->query($s)){
				if($r=$q->row()){
					$category=$r->category;
					$supplier=$r->supplier_number;
					$manufacturer=$r->manufacturer;
				}
			}
			if($category!=""){
				$s="select p.nilai,p.nilai2,p.nilai3,p.tipe,p.promosi_code
				from promosi_item i 
				join promosi_disc p on p.promosi_code=i.promosi_code where '".date('Y-m-d H:i:s')."' 
				between p.date_from and p.date_to and p.category='1'  and p.tipe='2'  and p.flag1='0' 
				and i.item_number='$category' and i.item_type in ('category','kode_category')";	
				if($q=$this->db->query($s)){
					if($r=$q->row()){
						$disc1=$r->nilai;
						$disc2=$r->nilai2;
			...
			if($supplier!="" && $disc1==0){
				...
				join promosi_disc p on p.promosi_code=i.promosi_code where '".date('Y-m-d H:i:s')."' 
				between p.date_from and p.date_to  and p.category='1' and p.tipe='2' and p.flag1='0' 
				and i.item_number='$supplier' and i.item_type in ('supplier','kode_supplier')";	
				if($q=$this->db->query($s)){
					if($r=$q->row()){
						$disc1=$r->nilai;
						$disc2=$r->nilai2;
			...
			if($manufacturer!="" and $disc1==0){
				$s="select p.nilai,p.nilai2,p.nilai3,p.tipe,p.promosi_code 
				from promosi_item i 
				join promosi_disc p on p.promosi_code=i.promosi_code where '".date('Y-m-d H:i:s')."' 
				between p.date_from and p.date_to  and p.category='1' and p.tipe='2' and p.flag1='0' 
				and i.item_number='$manufacturer' and i.item_type in ('merk','kode_merk')";	
				if($q=$this->db->query($s)){
					if($r=$q->row()){
						$disc1=$r->nilai;
						$disc2=$r->nilai2;
		...
		$retval['disc_prc_1']=$disc1;
		$retval['disc_prc_2']=$disc2;
		$retval['disc_prc_3']=$disc3;
		$retval['promosi_code']=$prcode;
		$retval['tipe']=$ptype;
	...
	function import_excel(){
		...
		if($c_cost=="")$c_cost=$c_beli;
		
		$c_jual1=$this->input_col('jual1');
		$t_jual1=$this->input->post('jual1_kel');
		if(trim($t_jual1)=="")$c_jual1=0;

		$c_jual2=$this->input_col('jual2');
		$t_jual2=$this->input->post('jual2_kel');
		if(trim($t_jual2)=="")$c_jual2=0;
		
		$c_jual3=$this->input_col('jual3');
		$t_jual3=$this->input->post('jual3_kel');
		if(trim($t_jual3)=="")$c_jual3=0;
		...
		if($_FILES["file_excel"]["size"] > 0)
		{
			...
			while (($emapData = fgetcsv($file, 10000, chr(9))) !== FALSE)
			{
				...
				if(! ($item_no == null or $item_no == "" or $item_no == "kode" ) ) {
					...
					if($c_unit>0 && count($emapData)>$c_unit)$data["unit_of_measure"]=$emapData[$c_unit];
					if($c_jual>0 && count($emapData)>$c_jual)$data["retail"]=$emapData[$c_jual];
					if($c_cost>0 && count($emapData)>$c_cost)$data["cost"]=c_($emapData[$c_cost]);
					if($c_beli>0 && count($emapData)>$c_beli)$data["cost_from_mfg"]=c_($emapData[$c_beli]);
					if(isset($data['cost']))if(c_($data['cost'])==0)$data['cost']=$data['cost_from_mfg'];
					
					if($c_class>0 && count($emapData)>$c_class)$data["class"]=$emapData[$c_class];
					if($data['class']=="")$data["class"]="Stock Item";

					if($c_categori1>0 && count($emapData)>$c_categori1)$data["category"]=$emapData[$c_categori1];
					if($c_categori2>0 && count($emapData)>$c_categori2)$data["sub_category"]=$emapData[$c_categori2];
					if($c_categori3>0 && count($emapData)>$c_categori3)$data["sub_cat_1"]=$emapData[$c_categori3];
					if($c_supplier>0 && count($emapData)>$c_supplier)$data["supplier_number"]=$emapData[$c_supplier];
					if($c_division>0 && count($emapData)>$c_division)$data["division"]=$emapData[$c_division];
                    if($c_manufacturer>0 && count($emapData)>$c_manufacturer)$data["manufacturer"]=$emapData[$c_manufacturer];
                    if($c_model>0 && count($emapData)>$c_model)$data["model"]=$emapData[$c_model];
                    if($c_type_of_invoice>0 && count($emapData)>$c_type_of_invoice)$data["type_of_invoice"]=$emapData[$c_type_of_invoice];
                    if($c_qty>0 && count($emapData)>$c_qty)$data["quantity_in_stock"]=c_($emapData[$c_qty]);
                    if($c_colour>0 && count($emapData)>$c_colour)$data["colour"]=$emapData[$c_colour];
                    if($c_size>0 && count($emapData)>$c_size)$data["size"]=$emapData[$c_size];
                    if($c_kode_lama>0 && count($emapData)>$c_kode_lama)$data["kode_lama"]=$emapData[$c_kode_lama];
					$data['cid']=cid();
                    //$data['insr_name']=1;
					...
					if($c_jual1>0){
						$this->save_price_cust($item_no,$t_jual1,$emapData[$c_jual1]);
					}
					if($c_jual2>0){
						$this->save_price_cust($item_no,$t_jual2,$emapData[$c_jual2]);
					}
					if($c_jual3>0){
						$this->save_price_cust($item_no,$t_jual3,$emapData[$c_jual3]);
					}
				}
			}
	...
	function save_price_cust($item_no,$cust_type,$sales_price){
		if(trim($cust_type)=="" || c_($sales_price)==0)return false;
		$data['item_no']=$item_no;
		$data['cust_type']=$cust_type;
		$data['sales_price']=$sales_price;
		$this->inventory_price_customers_model->save($data);
	...
	function print_barcode_add(){
		$item_number=$this->input->get("item_number");
		
		$description="";
		$retail=0;			$supplier_number="";		$category="";		$sub_category="";
		$cost=0;			$cost_from_mfg=0;			$supplier_name="";	$model="";
		$manufacturer="";	$upc_code="";				$manufacturer_item_number="";
		$custom_field_1="";	$custom_field_2="";			$unif_of_measure="";
		$custom_field_3="";	$unit_of_measure="";
		$s="select i.item_number,i.description,i.supplier_number,s.supplier_name,
			i.category,i.sub_category,i.retail,i.cost,i.cost_from_mfg,i.model,
			i.manufacturer,i.upc_code,i.manufacturer_item_number,i.unit_of_measure,
			i.custom_field_1,i.custom_field_2,i.custom_field_3
		...
		if($q=$this->db->query($s)){
			if($r=$q->row()){
				...
				if($cost=="")$cost=0;
				if($cost_from_mfg=="")$cost_from_mfg=0;
				if($retail=="")$retail=0;
		...
		$custom_field_2=$terminal;
		
		$ukuran=$this->input->get("ukuran");
		$custom_field_3=$ukuran;

		$s="insert into zzz_barcode set item_number='$item_number',
			description='$description',
			retail='$retail',cost='$cost',cost_from_mfg='$cost_from_mfg',
			supplier_number='$supplier_number',supplier_name='$supplier_name',
			category='$category',sub_category='$sub_category',
			model='$model',manufacturer='$manufacturer',
			upc_code='$upc_code',unit_of_measure='$unit_of_measure',
...
//			$pdf->AddPage();
	...
	function recalc($item_number="",$tahun="",$bulan=""){
		$msg="recalc_stock_arsip_gudang \r";	
        ...
        if($item_number=="") {
			$msg.="\r inventory: recalc: next item_number is empty";
...
//		$this->inventory_model->trx_data_sj("T00");
		...
		if($msg=="")$msg="Ready.";		
		$this->inventory_model->next_item_recalc(true);
...
function login($cmd){
 ...
 function check_database($password)  {
   ...
   if($result)  {
	...
	function last_item_no(){
		$success=false;
		$message="";
		$order_by=" order by item_number desc";
		$item_no=$this->input->get("item_number");
		if($order_by_get=$this->input->get("sort")){
			$order_by=$order_by_get;
		}		
		$description="";
		$s="select item_number,description from inventory 
		where item_number like '$item_no%'  $order_by ";
