...
//            $data['payment_terms_list']=$this->type_of_payment_model->select_list();
    ...
    function save_item(){
		...
		if(isset($data['disc_amount_1a'])){
			$data['discount_amount']=$data['disc_amount_1a'];
			unset($data['disc_amount_1a']);
		}
		//var_dump($data);
...
//        $data['payment_terms_list']=$this->type_of_payment_model->select_list();
    ...
    function browse($offset=0,$limit=50,$order_column='sales_order_number',$order_type='asc'){
		...
		$data['fields_caption']=array('Nomor Faktur','Tanggal','Termin','Jumlah',
			'Saldo','Paid','Customer','Nama Customer',
			'Salesman','Gudang','Ref','Due Date');
		$data['fields']=array('invoice_number','invoice_date','payment_terms','amount', 
            'saldo_invoice','paid','sold_to_customer','company',
			'salesman','warehouse_code','sales_order_number','due_date');
        ...
        $data['fields_format_numeric']=array("amount",'saldo_invoice');
    ...
    function e_faktur($nomor){
		...
		$data['ship_to_customer']=$invoice->ship_to_customer;
		$data['salesman']=$invoice->salesman;
        $data['content']=load_view('sales/rpt/e_faktur',$data);    	
        $this->load->view('pdf_print',$data);    	
    }
...
//					$row->invoice_date,$row->due_date,$row->payment_terms));
	...
	function grafik_jual_harian(){
		...
		sum(coalesce(p.amount,p.inv_amount)) as sum_amount 
		from invoice p
	...
	function sales_by_date()
	{
		$tahun=date("Y");
		$bulan=date("m");
		$sql="select DATE_FORMAT(`invoice_date`,'%Y-%m-%d') as tanggal,
		sum(if(invoice_type='R',-1,1)*p.inv_amount) as amount 
		from invoice p
		where invoice_type in ('I','R') and year(invoice_date)='$tahun'
			and month(invoice_date)='$bulan' 
		group by DATE_FORMAT(`invoice_date`,'%Y-%m-%d')
	...
	function daftar_saldo_faktur()
	{
		$tahun=date("Y");
		$bulan=date("m");
		$sql="select invoice_number,invoice_date,coalesce(amount,inv_amount) as amount,
		company,due_date  
		from invoice i left join customers c on c.customer_number=i.sold_to_customer
		where invoice_type='I' and year(i.invoice_date)='$tahun'
			and month(i.invoice_date)='$bulan' 
	...
	function omzet_salesman() {
		$tahun=date("Y");
		$bulan=date("m");
		$sql="select salesman,sum(coalesce(amount,inv_amount)) as jumlah 
		from invoice i 
		where invoice_type='I' and year(i.invoice_date)='$tahun'
			and month(i.invoice_date)='$bulan' 
		group by salesman 
	...
	function omzet_outlet(){
		$tahun=date("Y");
		$bulan=date("m");
	    $sql="select il.warehouse_code as kode,sum(coalesce(il.amount,0)) as jumlah 
	    from invoice_lineitems il 
		left join invoice i on i.invoice_number=il.invoice_number 
	    where i.invoice_type='I' and year(i.invoice_date)='$tahun'
			and month(i.invoice_date)='$bulan' 
	...
	function sales_by_customer() {
		$tahun=date("Y");
		$bulan=date("m");
		$sql="select c.company,sum(coalesce(i.amount,i.inv_amount)) as jumlah 
		from invoice i left join customers c on c.customer_number=i.sold_to_customer
		where invoice_type='I'  and year(i.invoice_date)='$tahun'
		and i.sold_to_customer<>'CASH'
			and month(i.invoice_date)='$bulan' 
		group by c.company 
		order by sum(ilamount) desc
		limit 0,31";
		echo datasource($sql);
	}
	function items($nomor,$type='')
	{
		...
		p.no_urut,p.from_line_type,p.from_line_doc,p.discount_amount,p.disc_amount_2,p.disc_amount_3 
		from invoice_lineitems p
		left join inventory i on i.item_number=p.item_number
	...
	function recalc_run(){
		$invoice_no="";
		if($q=$this->db->get('zzz_invoice_need_update')){
			$r=$q->row();
			if($r)$invoice_no=$r->invoice_no;
		}
		$this->db->where("invoice_no",$invoice_no)->delete("zzz_invoice_need_update");
		if($invoice_no!=""){
			$this->recalc($invoice_no);	
		}
		echo "invoice.recalc_run: $invoice_no";
	}
	function recalc_point($invoice_number,$debug=false){
		$this->load->model('sales/promosi_point_model');
		$this->promosi_point_model->recalc_invoice($invoice_number,$debug);
	}
