...
class Jobs extends CI_Controller {
    ...
    private $sql="select user_group_id,user_group_name,description from modules_groups i ";
...
//		echo $sql;
		...
		foreach($modules->result() as $row){
			...
			";	
			if($this->modules_groups_model->has_child()){
				$tbl .= $this->level2($row,$filter,$group_id);
			} else {
				$checked=$this->modules_groups_model->checked();

				$tbl.="<tr>
				<td><input type='checkbox' name='modules[]' value='".$row->module_id."' $checked  
				class='checkbox' style='width:30px'></td>
...
//								<td>".link_button("Edit","edit_module('".$lvl2->module_id."')","edit")."</td>
	...
	function check($group_id,$module_id,$rpt_mod="",$rpt_rpt="",$rpt_file=""){
		$group_id=urldecode($group_id);
		$module_id=urldecode($module_id);
		if($rpt_mod!=""){
			$rpt_mod=urldecode($rpt_mod);
		}
		if($rpt_rpt!=""){
			$rpt_rpt=urldecode($rpt_rpt);
		}
		if($rpt_file!="")$rpt_file=urldecode($rpt_file);
		if($rpt_mod!="" && $rpt_rpt!="" ){
			$module_id=$module_id."/".$rpt_mod."/".$rpt_rpt;

		}
		$this->modules_groups_model->add_module($group_id,$module_id);
	}
	function un_check($group_id,$module_id,$rpt_mod="",$rpt_rpt="",$rpt_file=""){
		$group_id=urldecode($group_id);
		$module_id=urldecode($module_id);
		if($rpt_mod!=""){
			$rpt_mod=urldecode($rpt_mod);
	...
	function user_list($user_group_id){
		$s="select uj.user_id,u.username,u.cid 
		from user_job uj 
		left join user u on u.user_id=uj.user_id
		where uj.group_id='$user_group_id'";
		echo datasource($s);
	}
}
