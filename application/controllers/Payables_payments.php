...
class Payables_payments extends CI_Controller {
    ...
    private $sql="select purchase_order_number,i.terms,po_date,amount, 
            i.supplier_number,c.supplier_name,c.city,i.warehouse_code,i.cid
    ...
    function browse($offset=0,$limit=50,$order_column='purchase_order_number',$order_type='asc'){
		...
		$data['fields_caption']=array('Nomor Bukri','Tgl Bayar','Faktur',
			'Tgl Faktur','Jenis','Jumlah Faktur',
			'Jumlah Bayar','Kode Supplier','Nama Supplier','Kota','Line','Cid');
		$data['fields']=array('no_bukti','date_paid','purchase_order_number',
			'po_date','how_paid','amount',
   ...
   function view($no_bukti,$message=""){
	...
	function print_doc($nomor){
		$nomor=urldecode($nomor);		
		$data['payment']=$this->payables_payments_model->get_by_id($nomor)->row();
		$data['content']=load_view("purchase/rpt/print_payment",$data);		
