 ...
 */
   ...
   function add_payment(){
		...
		$mode_pay=$this->input->post('mode_pay');
		if($mode_pay=="add_so_payment"){
			$invoice=$this->db->select("sold_to_customer,account_id")
			->where("sales_order_number",$invoice_no)
			->get("sales_order")->row();
   ...
   function delete_no_bukti($no_bukti)
   {
	...
	function print_doc($nomor){
		$nomor=urldecode($nomor);		
		$data['payment']=$this->payment_model->get_by_id($nomor)->row();
		$data['content']=load_view("sales/rpt/print_payment",$data);		
