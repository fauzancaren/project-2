...
class Purchase_invoice extends CI_Controller {
	...
	function __construct()
	{
		...
		$this->sql="select i.purchase_order_number,i.terms,i.po_date,i.amount,i.posted, 
            i.supplier_number,c.supplier_name,c.city,i.warehouse_code,i.po_ref,
            (i.amount-coalesce(p.sum_paid,0)) as saldo,i.type_of_invoice,i.due_date,i.paid
	...
	function save(){
		...
		if ($ok){
			...
			echo json_encode(array('msg'=>'Some errors occured.'.show_error('purchase_order.save')));
		}
	}
    ...
    function browse($offset=0,$limit=50,$order_column='purchase_order_number',$order_type='asc'){
		...
		$data['fields_caption']=array('Nomor Faktur','Tanggal','Jumlah','Saldo','Termin',
		'Due Date','Paid','Posted','Kode Supplier',
		  'Nama Supplier','Kota','Gudang','Recv No','Sistim');
		$data['fields']=array('purchase_order_number','po_date','amount','saldo','terms',
		'due_date','paid','posted', 
    ...
    function browse_data($offset=0,$limit=10,$nama=''){
    	...
    	if($this->input->get('sid_po_number')!=''){
    		$sql.=" and i.purchase_order_number='".$this->input->get('sid_po_number')."'";
		} else {
			if($this->input->get('sid_date_from')){
				$d1= date( 'Y-m-d H:i:s', strtotime($this->input->get('sid_date_from')));
				$d2= date( 'Y-m-d H:i:s', strtotime($this->input->get('sid_date_to')));			 
				$sql.=" and i.po_date between '".$d1."' and '".$d2."'";	
			}
        ...
        if($search!="")$sql.=" and i.purchase_order_number like '$search%'";
...
//			echo $this->payables_payments_model->browse($purchase_order_number);	
            ...
            if($bill!=0){
                ...
                $sql="select * from payables_payments where purchase_order_number='$purchase_order_number'";
            }
		...
		{
			$nomor_bukti=urldecode($nomor_bukti);
    ...
    function add_item_with_po($purchase_order_number,$invoice_number)
    {
		...
		$due_date="";
		$terms="";
        if($qpo=$this->purchase_order_model->get_by_id($po_ref)){
			if($rpo=$qpo->row()){
				$due_date=$rpo->due_date;
				$terms=$rpo->terms;
			}
		}
        foreach($query->result() as $row)
		...
		$s="update purchase_order set po_ref='$purchase_order_number' ";
		if($terms!="")$s.=",terms='$terms'";
		if($due_date!="")$s.=",due_data='$due_date'";
		$s.="where purchase_order_number='$invoice_number' and potype='I'";
		$this->db->query($s);
