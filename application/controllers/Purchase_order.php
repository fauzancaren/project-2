...
class Purchase_order extends CI_Controller {
	...
	function __construct()
	{
		...
		$this->sql="select purchase_order_number,po_date,
	...
	function save(){
		...
		if ($ok){
			...
			$msg = show_error("purchase_order.save()");
	...
	function update()
	{
		 ...
		 if ($this->form_validation->run()=== TRUE){
			...
			$message = show_error("purchase_order.save()");
		}
			...
			if($nomor){
				...
				if($query){ 
					foreach($query->result_array() as $row){
						$qty_recvd=$row['qty_recvd'];
						if ($qty_recvd=="")$qty_recvd=0;
						$qty_sisa=$row['quantity']-$qty_recvd;
						if($row['quantity']>0){
...
//      if(isset($data['profit']))unset($data['profit']);
	...
	function recalc_run(){
		...
		if($q=$this->db->get('zzz_purchase_need_update')){
			$r=$q->row();
			if($r)$purchase_order_number=$r->invoice_no;
		}
		$this->db->where("invoice_no",$purchase_order_number)->delete("zzz_purchase_need_update");
		if($purchase_order_number!=""){
			$this->recalc($purchase_order_number);	
		}
		$this->load->model('purchase_invoice_model');
		$this->purchase_invoice_model->recalc($purchase_order_number);
		echo "Purchase_order.recalc_run: $purchase_order_number";
	}
