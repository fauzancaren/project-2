...
class Purchase_request extends CI_Controller {
	...
	function __construct()
	{
		...
		$this->sql="select purchase_order_number,po_date,i.project_code,
                i.branch_code,i.dept_code,i.ordered_by,i.doc_status,i.cid
	...
	function save(){
		...
		if ($ok){
			...
			$err=$this->db->error();
			$msg=$err['message'];
			echo json_encode(array('msg'=>'Some errors occured. $msg'));
		}
	}
    ...
    function browse($offset=0,$limit=50,$order_column='purchase_order_number',$order_type='asc',
		...
		$data['fields_caption']=array('Nomor PRQ','Tanggal','Proyek','Cabang',
			'Department','Pegawai','Status','Cid');
		$data['fields']=array('purchase_order_number','po_date','project_code', 
                'branch_code','dept_code','ordered_by','doc_status','cid');
	...
	function sub_total($nomor){
		...
		$data=array('sub_total'=>$sub_total,'amount'=>$this->purchase_order_model->amount,"msg"=>show_error('sub_total'));
