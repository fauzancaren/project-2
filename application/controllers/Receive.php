...
//            $data['item_number_list']=$this->inventory_model->item_list();
    ...
    function save_header(){
		...
		$message="Error";
		if($shipment_id!=""){
			$d['ref1']=$this->acc_id($data["ref1"]);
			$d['date_received']=$data['date_received'];
			$d['warehouse_code']=$data['warehouse_code'];
			$d['supplier_number']=$data['supplier_number'];
			$d['comments']=$data['comments'];
			$success=$this->db->where("shipment_id",$shipment_id)
				->update("inventory_products",$d);
