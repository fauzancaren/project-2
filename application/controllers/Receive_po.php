...
class Receive_po extends CI_Controller {
	...
	function __construct()
	{
		...
		$this->sql="select distinct ip.shipment_id, ip.date_received,ip.purchase_order_number, 
	...
	function view($id,$message=null){
		...
		$ShowItemCost=allow_mod("ShowItemCost");
		$data['ShowItemCost']=$ShowItemCost;
		$data['mode']='view';
...
//        $ok=$this->db->where("shipment_id",$shipment_id)->delete("inventory_products");
        $this->purchase_order_model->recalc_qty_recvd($nomor_po);
		$this->syslog_model->add($shipment_id,"receive_po","delete");
	...
	function save_item(){
		...
		if($data=$this->input->post()){
			...
			$success=$this->inventory_products_model->update($id,$data);
	...
	function del_item($id,$recv_id,$is_ajax=false){
		$id=urldecode($id);
		$recv_id=urldecode($recv_id);
        $this->db->where('id',$id)->where("shipment_id",$recv_id);
		$success = $this->db->delete('inventory_products');
		$msg="";
		if($is_ajax){
			$msg="Success, baris sudah dihapus.";
	...
	function proses()
	{
		...
		//php.ini seting kalo receive diatas 100 baris: max_input_vars=4000
		// 												post_max_size=40M
		//var_dump($d);exit;
//		var_dump(count($qty));exit;
		...
		$this->syslog_model->add($id,"receive_po","add");
