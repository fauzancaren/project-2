...
class Sales_order extends CI_Controller {
	...
	function __construct()
	{
        ...
        $this->load->library(array('sysvar','javascript','template','form_validation'));
		$this->load->model(array('sales_order_model','customer_model','inventory_model',
			'type_of_payment_model','salesman_model','syslog_model','customer_shipto_model'));

		$this->sql="select i.sales_order_number,i.sales_date,i.due_date,i.amount, 
	...
	function set_defaults($record=NULL)
	{
			...
			if(date("Y",strtotime($data['due_date']))<2021){
//				$data['due_date']=due_date($data['sales_date'],$data['payment_terms']);
				$data['due_date']=$data['sales_date'];
			}   
			...
			$data['allow_print']=1;	//allow_mod('_30056');
            ...
            $data['lookup_shipto']=$this->customer_shipto_model->lookup();
	...
	function save()	{ 
		...
		if(isset($data['disc_total_percent'])){
			$data['discount']=$data['disc_total_percent']; 
			unset($data['disc_total_percent']);
		}
		if(isset($data['disc_total_amount_1'])){
			$data['disc_amount_1']=c_($data['disc_total_amount_1']); 
			unset($data['disc_total_amount_1']);

		}
		if(isset($data['total'])){
			$data['amount']=c_($data['total']); 
			unset($data['total']);

		}
		if(isset($data['tax'])){
			$data['tax']=c_($data['tax']);
		}
		unset($data['mode']);
	...
	function view($id,$message=null){
		...
		$this->sales_order_model->recalc_ship_qty($id);
		 ...
		 $customer_info=$this->customer_model->info($data['sold_to_customer']);
		 if($data['sold_to_customer']!=$data['ship_to_customer']){
			$customer_info2=$this->customer_shipto_model->ship_info($data['ship_to_customer']);
			if($customer_info2!=""){
	...
	function items($nomor,$type='',$no_echo=false)
	{
		...
		p.warehouse_code,s.salesman,p.discount_amount,p.disc_amount_2,p.disc_amount_3
		from sales_order_lineitems p
		left join inventory i on i.item_number=p.item_number
    ...
    function save_item(){
		...
		$amount=$qty*$price;
		$disc1=c_($this->input->post('discount'));
		if($disc1>1)$disc1=$disc1/100;
		$disc_amt_1=c_($this->input->post('disc_amount_1'));
		if($disc1>0){
			$disc_amt_1=$amount*$disc1;
		}
		$amount=$amount-$disc_amt_1;
		
		$disc2=c_($this->input->post('disc_2'));
		if($disc2>1)$disc2=$disc2/100;
		$disc_amt_2=$amount*$disc2;
		$amount=$amount-$disc_amt_2;

		$disc3=c_($this->input->post('disc_3'));
		if($disc3>1)$disc3=$disc3/100;
		...
		if($qty_extra=$this->promosi_model->promo_qty_extra($item_no,$data['quantity'])){
			if($qty_extra>0){
				if($result=$this->promosi_model->result){
					if($result['issameitem']!='0'){
						...
						if($item_free=$result['item_free']){
							if($item_no=$item_free['item_number']){
								$this->load->model("inventory_model");
								if($qitem=$this->inventory_model->get_by_id($item_no)){
									if($rdata=$qitem->row()){
										$data['item_number']=$rdata->item_number;
										$data['description']="*** extra x ".$rdata->description;
										$data["quantity"]=$qty_extra;
										$data['price']=0;				$data['amount']=0;
										$data['discount']=0;			$data['discount_amount']=0;
										$data['disc_2']=0;				$data['disc_amount_2']=0;
		...
		if($item->assembly){
			...
			if($q=$this->inventory_assembly_model->get_by_parent($item->item_number)){
				foreach($q->result() as $rasm){
					$data['item_number']=$rasm->assembly_item_number;
					$data['description']="***ASM: ".$rasm->description;
					$data["quantity"]=$qty*$rasm->quantity;
					$data['price']=0;				$data['amount']=0;
					$data['discount']=0;			$data['discount_amount']=0;
					$data['disc_2']=0;				$data['disc_amount_2']=0;
					$data['disc_3']=0;				$data['disc_amount_3']=0;
	...
	function recalc_run(){
		$sales_order_number="";
		if($q=$this->db->get('zzz_sales_order_need_update')){
			$r=$q->row();
			if($r)$sales_order_number=$r->sales_order_number;
		}
		$this->db->where("sales_order_number",$sales_order_number)->delete("zzz_sales_order_need_update");
		if($sales_order_number!=""){
			$this->recalc($sales_order_number);	
		}
		echo "sales_order.recalc_run: $sales_order_number";
	}
    ...
    function print_so($nomor){
		...
		$data['ship_to_customer']=$invoice->ship_to_customer;
    ...
    function print_bo($nomor){
        ...
        $data['content']=load_view('sales/rpt/print_bo',$data);
        $this->load->view('pdf_print',$data);
   	}
