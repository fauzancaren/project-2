...
class Sysvar_data extends CI_Controller {
   ...
   function browse($offset=0,$limit=50,$order_column='varname',$order_type='asc'){
		...
		$data['fields_caption']=array('VarName','VarValue','Keterangan','Category',
            'Section','VarType','VarList','ID','CID');
		$data['fields']=array('varname','varvalue','keterangan','category','section',
            'vartype','varlist','id','cid');
    ...
    function browse_json($offset=0,$limit=100,$nama=''){
        $this->browse_data($offset,$limit,$nama);
    }
    function browse_data($offset=0,$limit=100,$nama=''){
        ...
        if(isset($_POST['page']))$offset=$_POST['page'];
        if(isset($_POST['rows']))$limit=$_POST['rows'];
        if($offset>0)$offset--;
        $offset=$limit*$offset;
    ...
    function copy_data($cid_to,$cid_from=''){
        $f="varname,varvalue,keterangan,varlen,section,category,vartype,varlist,plus_minus";
        $s="insert into system_variables($f,cid) 
            select $f,'$cid_to'  
            from system_variables ";
        if($cid_from==""){
            $s.=" where cid is null";
        } else {
            $s.=" where cid=$cid_from'";
        } 
        $this->db->query($s);
        $data['success']=true;
        $data['msg']='Success';
