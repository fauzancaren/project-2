...
class Aktiva_group extends CI_Controller {
	...
	function __construct()
	{
		...
		$this->sql="select id,name,at_cost,accum_depn,profit_on_sale,loss_on_sale,cash_bank
	...
	{
		$key="FA GROUP Numbering";
        $no=$this->sysvar->autonumber($key,0,"!G~$001");
		if($add){
	...
	function save(){
		...
		if ($this->form_validation->run()=== TRUE){
			...
			unset($data['mode']);
			if($mode=="add"){
				$success=$this->aktiva_group_model->save($data);
				$data['message']=$this->db->error()['message'];
				if($data['message']==''){
					$data['mode']='view';					
				...
				$success=$this->aktiva_group_model->update($id,$data);
				$data['message']=$this->db->error()['message'];
				if($data['message']==''){
					$data['message']='Data sudah disimpan.';
				}
	...
	function delete($id){
		...
		$message="";
		$success=false;
		$s="select description from fa_asset where group_id='$id'";
		if($q=$this->db->query($s)){
			if($r=$q->row()){
				$message="Tidak bisa hapus karena masih ada asset [$r->description]";
				echo json_encode(array("success"=>$success,"message"=>$message));
			}
		}
		$success=$this->aktiva_group_model->delete($id);
