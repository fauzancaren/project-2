...
class Aktiva_purchase extends CI_Controller {
	...
	function __construct()
	{
		...
		$this->sql = "select f.journal_id,trans_date,asset_id,trans_value ,f.trans_type,
	...
	function save(){
		...
		if ($this->form_validation->run()=== TRUE){
			...
			if($mode=="add"){
				$success=$this->aktiva_tran_model->save($data);
				...
				$success=$this->aktiva_tran_model->update($id,$data);
				$data['message']=$this->db->error()['message'];
				if($data['message']==''){
					$data['message']='Data sudah disimpan.';
    ...
    function browse_data($offset=0,$limit=100,$nama=''){
		...
		if($this->input->get('sid_number')!='')$sql.=" and f.journal_id like '".$this->input->get('sid_number')."%'";	
		if($this->input->get('sid_nama')!='')$sql.=" f.asset_id like '".$this->input->get('sid_nama')."%'";
