...
class Angsuran_setor extends CI_Controller {
	...
	function __construct()
	{
		...
		$this->load->model("koperasi/angsuran_model");
	...
	function save(){
		...
		if($data['mode']=="add"){
			$payment_no=$this->nomor_bukti();
		}
		$ok=$this->pinjaman_model->save_bayar();
		if($ok){
			$this->nomor_bukti(true);
			echo json_encode(array("success"=>true,"msg"=>"Success","payment_no"=>$payment_no));
		} else {
	...
	function view($id='',$message=null){	 
		 $id=urldecode($id);
		 $model=$this->angsuran_model->get_by_id($id)->row();
	...
	function browse($offset=0,$limit=10,$order_column='loan_number',$order_type='asc')	{
		...
		$data['fields_caption']=array('Tanggal','No Pinjaman','Bulan Ke','No Anggota',
		'Tagihan','Bayar','Nama Anggota','Id');
		$data['fields']=array('tanggal_bayar','no_pinjaman','bulan_ke','no_anggota',
    ...
    function browse_data($offset=0,$limit=10,$nama=''){
		$sql="select t.tanggal_bayar,t.no_pinjaman,t.bulan_ke,t.no_anggota
