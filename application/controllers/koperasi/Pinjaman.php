...
//		$data['jenis_pinjaman_list']=$this->jenis_pinjaman_model->item_list();
	...
	function cicilan($cmd,$id="") {
		$id=urldecode($id);
		if($cmd=="list") {
			$sql="select no_urut,tanggal_jth_tempo,awal,pokok,bunga,angsuran,akhir, 
	...
	function info($no_pinjaman=""){
		$bulan_ke=0;
		$success=false;
		$message="";
		$tanggal="";
		$angsuran=0;
		$s="select * from kop_cicilan where no_pinjaman='$no_pinjaman' and payment_no is null
		order by no_urut"; 
		if($q=$this->db->query($s)){
			if($r=$q->row()){
				$angsuran=$r->angsuran;
				$bulan_ke=$r->no_urut;
				$tanggal=$r->tanggal_jth_tempo;
				$success=true;
			}
		}

		$data['success']=$success;
		$data['tagihan']=$angsuran;
		$data['tanggal_tagih']=$tanggal;
