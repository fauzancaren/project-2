...
private $limit=10;
    ...
    function posting($nomor){
		$success=false;
		$message="Success";
		$success=$this->mat_release_model->posting($nomor);
		if(!$success){
			$message="Tidak bisa posting $nomor";
		} else {
			$this->db->query("update mat_release_header set posted=1 where mat_rel_no='$nomor'");
		}
	...
	function unposting($nomor){
		$success=false;
		$message="Success";
		$success=$this->mat_release_model->unposting($nomor);
		$data["success"]=$success;
		$data["message"]=$message;
		echo json_encode($data);
	}
