...
class Receive_prod extends CI_Controller {
    ...
    function browse_data($offset=0,$limit=10,$nama=''){
		...
		if($no!=''){
			...
			//if($nama!='' || $nama!='x')$sql.=" and s.supplier_name like '$nama%'";	
		}
		if($offset>0)$offset--;
	...
	function posting($nomor){
		$success=$this->inventory_products_model->posting($nomor);
		$message="Success";
	...
	function unposting($nomor){
		$success=$this->inventory_products_model->unposting($nomor);
		$message="Success";
		$data['success']=$success;
		$data['message']=$message;
		echo json_encode($data);
