...
class Workorder extends CI_Controller {
	...
	function set_defaults($record=NULL){
		...
		$param['dlgRetFunc']="$('#customer_number').val(row.sold_to_customer)";
		$data['so_lookup']=$this->sales_order_model->lookup($param);
	...
	function delete($id,$isJson=false){
		$message="Success";
		$success=$this->workorder_model->delete($id);
		if($isJson){
	...
	function items($nomor)
	{
		...
		if(p.price=0 or p.price is null,i.cost,p.price) as price,
		if(p.price=0 or p.price is null,p.quantity*i.cost,p.total) as total,
		p.qty_exec,p.qty_bal
