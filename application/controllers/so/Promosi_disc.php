...
class Promosi_disc extends CI_Controller {
    ...
    function set_defaults($record=NULL){
		...
		$data['lookup_inventory']=$this->inventory_model->lov("inventory",";console.log(row);",true);
...
//			$sql.="  between '$d1' and '$d2'";
	...
	function items_extra($nomor){
		$sql="select * from promosi_extra_item where promosi_code='$nomor' order by id";
		echo datasource($sql);
	}
	...
	function delete_item_extra($id){
		$id=urldecode($id);
		 $ok=$this->promosi_model->delete_item_extra($id);
		 echo json_encode(array("success"=>$ok));
	}
	function save_item(){
		$id=0;
	...
	function save_item_extra(){
		...
		if($item_number && $promosi_code){
			...
			//$data['line_type']=$item_type;
			//$data['item_type']=$item_type;
			$id= $this->promosi_model->save_item_extra($data);	
		}
		if($id>0)$success=true;
		echo json_encode(array("success"=>$success,"id"=>$id));
	}
