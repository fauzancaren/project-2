...
if(!function_exists("null_text_clear")){
	function null_text_clear($data){
		$retval=null;
...
if(!function_exists("show_error")){
	function show_error($module=""){
		$CI=&get_instance();		
		$msg='Some error occured.';
		if($module!='')$msg.=', module: '.$module;
		if($err=$CI->db->error()){
			$msg.=", err: ".$err['code'];
			$msg.=", desc: ".$err['message'];
		}
			
		$CI->syslog_model->add("module: ".$module,$msg,"error");
		return $msg;
	}
}
...
if(!function_exists("show_only_current_sid")){
	function show_only_current_sid(){
		$CI=&get_instance();
		return $CI->config->item("show_only_current_sid");
	}
}
if(!function_exists("multi_company_single_db")){
	function multi_company_single_db(){
		$CI =& get_instance();		
		$retval=$CI->config->item("multi_company_type")=="1"; 
		return $retval;
	}
}
...
if(!function_exists("add_button_menu")){
	function add_button_menu($Vyfsyvpoqvi0,$Vpckg22z3gi4,$Vu0pjn5k3qbkco,$Vcoivezkelba,$Vqlidxeiorll=""){
		...
		if($themes=="admin"){
			$class_click="button-menu";
		} else {
			$class_click="info_link";
		}
	...
	echo "<div class='col-lg-3 col-md-4 col-sm-12 info-maxon 
		thumbnail $class_click box-gradient ' $Vr24ddw5fsns>
...
if(!function_exists("user_flag2")){
	function user_flag2(){
        $ci =& get_instance();
		$retval=false;
		$uid=user_id();
		if($q=$ci->db->query("select flag2 from user where user_id='$uid'")){
			if($r=$q->row()){
				$retval=$r->flag2;
...
if(!function_exists("current_gudang")){
    function current_gudang(){
	   ...
	   if($retval=="" || $retval=="UNKNOWN"){
		if($q=$CI->db->query("select location_number from shipping_locations")){
			if($r=$q->row()){
				$retval=$r->location_number;
			}
		}
...
if(!function_exists("cid")){
	function cid(){
		...
		$cid=$CI->access->cid();  
        if($multi_company && !multi_company_single_db()){
...
if(!function_exists('link_button')){
    function link_button($caption,$func,$icon='',$plain='false',$url='',$title='',$id='',$class='',$extra=''){
    	$idd="";
		...
		if(getvar("themes")=="admin"){
			if($class==""){
				$class="btn-sb btn-primary-sb";
			}
			if($caption=="")$caption="Pilih";
			if($caption=="Close")$class="btn-sb btn-danger-sb";
			if($caption=="Save")$class="btn-sb btn-success-sb";
		}

    	if($url==''){
	        return '<a href="#" class="easyui-linkbutton '.$class.' " id='.$id.' 
	        data-options="iconCls:\'icon-'.$icon.'\',
	        plain: '.$plain.'" onclick="'.$func.';return false;" '.$extra.' >'.$caption.'</a>';
		} else {
	        return '<a href="'.$url.'" class="easyui-linkbutton '.$class.' " id='.$id.'
	        data-options="iconCls:\'icon-'.$icon.'\',
	        plain: '.$plain.'"  " '.$extra.' >'.$caption.'</a>';
		}
    }
}
    ...
    function allow_mod2($mod_id,$json_format=false){
        ...
        if($query->num_rows()){
            ...
            if($json_format){
                ...
                <br><h2>Module Id: <strong style='color:red'>$mod_id</strong></h2>
				</p> 
				</div>";
				
				$s="select count(1) as cnt from modules where module_id='$mod_id'";
				if($q = $CI->db->query($s)){
					$cnt=$q->row()->cnt;
					if($cnt==0){
						$s="insert into modules set module_id='$mod_id',module_name='$mod_id',type='Form',
						form_name='$mod_id',description='Isi nama module ini',parentid='_00000' ";
						$CI->db->query($s);	
					}
				}
...
if(!function_exists('sales_order_need_update')){
	function sales_order_need_update($sales_order_number){
	   $CI =& get_instance();
	   $sales_order_number=urldecode($sales_order_number);
	   if($sales_order_number!="") {
		   if($q=$CI->db->where("sales_order_number",$sales_order_number)
		   ->get("zzz_sales_order_need_update")){
			   if($q->num_rows()==0){
				   $CI->db->insert("zzz_sales_order_need_update",
				   array("sales_order_number"=>$sales_order_number));
			   }
		   }
	   }
	}
...
if(!function_exists('purchase_need_update')){
	function purchase_need_update($purchase_order_number){
	   $CI =& get_instance();
	   $purchase_order_number=urldecode($purchase_order_number);
	   if($purchase_order_number!="") {
		   if($q=$CI->db->where("invoice_no",$purchase_order_number)->get("zzz_purchase_need_update")){
			   if($q->num_rows()==0){
				   $CI->db->insert("zzz_purchase_need_update",array("invoice_no"=>$purchase_order_number));
			   }
		   }
	   }
	}
...
if(!function_exists('invoice_need_update')){
	function invoice_need_update($invoice_number){
	   $CI =& get_instance();
	   $invoice_number=urldecode($invoice_number);
	   if($invoice_number!="") {
		   if($q=$CI->db->where("invoice_no",$invoice_number)->get("zzz_invoice_need_update")){
			   if($q->num_rows()==0){
				   $CI->db->insert("zzz_invoice_need_update",array("invoice_no"=>$invoice_number));
			   }
		   }
	   }
	}
