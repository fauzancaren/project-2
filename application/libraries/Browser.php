...
class Browser
{
	...
	function render($as_output=false)
	{
		...
		$tool=$this->toolbar($page,$item_page_max,$limit,$show_delete_button);
		$output .=  $tool;	
		$output	.=	$result;
		$output .= 	$tool;
		...
		$output_script	.= '<script type="text/javascript" charset="utf-8" src="'.base_url().'js/browser.js?v3"></script>';
	...
	function toolbar($page=0,$item_page_max=10,$limit=50,$show_add_button=true) 
	{
		...
		<div class=\"btn-group\" role=\"group\" aria-label=\"Navigation Button\" >
			...
			type=\"button\" class=\"btn btn-default glyphicon glyphicon-list glyphicon \"></button>&nbsp
			...
			<button onclick=\"search_item();return false;\" class=\"btn btn-default 
				glyphicon glyphicon-filter\" 
				type=\"button\" title='Search record by text'>
			</button>
			<input name='txtSearch' id='txtSearch' placeholder='Enter search' class='' />
