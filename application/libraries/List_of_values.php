...
class List_of_values
{
	...
	function render($setting){
		 ...
		 * dlgUrlQuery: untuk lookup tersendiri dari controllernya, 
         *      misal: inventory/browse_data
		 ...
		 * filter: query param filter lain tambahan dari dlgUrlQuery 
         *      misal barang/browse_data?filter=value
		 * sysvar_lookup: varname dari tabel system_variables misal lookup_doc_type
		 * check_add_mode: false        //tampil simpan dulu
         */
        ...
        if(!isset($this->setting['check_add_mode'])) $this->setting['check_add_mode']=false;
		...
		if(isset($this->setting['dlgUrlQuery'])){			
            $url_data=$this->setting['dlgUrlQuery'];
			if(strtolower(substr($url_data,0,4))!="http"){
				$url_data=base_url("index.php/$url_data");
			}
		} else {
    ...
    function get_by_name($what,$search=""){        
        ...
        if($check1_value=="undefined")$check1_value="";
        ...
        switch($what){
			...
			case "group_modules":
				...
				if($search!="")$sql.=" and (module_name like '%$search%' or module_id='$search') ";
                if($sort_by!=""){
                    $sql.=" order by $sort_by";
                } else {
			...
			case "sales_order_open":
                ...
                if($search!="")$sql.=" and (p.sales_order_number='$search' 
                    or c.company like '$search%' or p.sold_to_customer like '$search%' )";
            ...
            case "purchase_order":
                ...
                if($sort_by=="1") {
					$sort_by="i.supplier_number";
				} else {
					$sort_by="s.supplier_name";
				}
                if($sort_by!=""){
                    $sql.=" order by $sort_by";
                } else {
...
//
            ...
            case "lookup_customer_type":
                $sql="select type_name,disc_prc from customer_type " ;
                if($search!="")$sql.=" and (type_name like '%$search%')";
                if($sort_by!=""){
                    $sql.=" order by $sort_by";
                } else {
            ...
            case ("user_id"):
                $sql="select user_id,username from user where 1=1 ";
				if(multi_company_single_db() && cid()!="ALL"){
					$sql.=" and cid='".cid()."'";
				}
            ...
            case('city'):
                ...
                break;
            case('salesman'):
                $sql="select salesman from salesman where 1=1 ";
				if(multi_company_single_db() && cid()!="ALL"){
					$sql.=" and cid='".cid()."'";
				}
                if($search!="")$sql.=" and salesman like '%$search%'";
                if($sort_by!=""){
                    $sql.=" order by $sort_by";
                } else {
            ...
            case('terms'):
            case('payment_terms'):
                $sql="select type_of_payment,days from type_of_payment where 1=1";
                if($search!="")$sql.=" and type_of_payment like '%$search%'";
                if($sort_by!=""){
                    $sql.=" order by $sort_by";
                } else {
            ...
            case('customer_record_type'):
                ...
                break;
            case('departments'):
                $sql="select dept_code,dept_name from departments where 1=1";
				if(multi_company_single_db() && cid()!="ALL"){
					$sql.=" and cid='".cid()."'";
				}
                ...
                break;
            case('divisions'):
                $sql="select div_code,div_name from divisions where 1=1";
				if(multi_company_single_db() && cid()!="ALL"){
					$sql.=" and cid='".cid()."'";
				}
            ...
            case("inventory_categories"):
                $sql="select kode,category from inventory_categories where 1=1";
				if(multi_company_single_db() && cid()!="ALL"){
					$sql.=" and cid='".cid()."'";
				}
                if($search!="")$sql.=" and kode like '%$search%'";
                if($sort_by!=""){
                    $sql.=" order by $sort_by";
                } else {
                    $sql.=" order by kode";
                ...
                break;      
            case("suppliers"):
                $sql="select supplier_number,supplier_name,first_name,phone,city,payment_terms 
				from suppliers where 1=1";
				if(multi_company_single_db() && cid()!="ALL"){
					$sql.=" and cid='".cid()."'";
				}
                if($search!="")$sql.=" and (supplier_name like '%$search%' 
					or supplier_number like '%$search%')";
                if($sort_by=="1") {
					$sort_by="supplier_number";
				} else {
            ...
            case "person":
            case("employee"):
                $sql="select nama,nip,dept,divisi,nip_id,emptype from employee  where 1=1";
				if(multi_company_single_db() && cid()!="ALL"){
					$sql.=" and cid='".cid()."'";
				}
                if($search!="")$sql.=" and (nama like '%$search%' or nip='$search' or nip_id='$search')";
                if($sort_by!=""){
                    $sql.=" order by $sort_by";
                } else {
                    $sql.=" order by nama";
            ...
            case "cost_account":
            case "inventory_account":    
                $sql="select account,account_description,group_type,id 
                from chart_of_accounts where 1=1";
                if($search!=""){
                	$sql.=" and (account like '$search%' or account_description like '%$search%')";
				}
				if(multi_company_single_db() && cid()!="ALL"){
					$sql.=" and cid='".cid()."'";
				}
            ...
            case "invoice":    
                ...
                break;                                
			case "emptype": 
				$sql="select kode,keterangan from hr_emp_level where 1=1";
				if(multi_company_single_db() && cid()!="ALL"){
					$sql.=" and cid='".cid()."'";
				}
				$sql.=" order by kode";
