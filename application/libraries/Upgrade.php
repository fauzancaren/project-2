...
class Upgrade
 ...
 //   echo $key." - ".$this->CI->sysvar->getvar($key);
    ...
    $tables=array("purchase_order","purchase_order_lineitems","payables","payables_items","payables_payments",
    ...
    $this->create_zzz_invoice_need_update();
    $this->create_zzz_purchase_need_update();
    $this->create_zzz_sales_order_need_update();

    $this->add_field("promosi_disc","disc_prc_1","double");
    $this->add_field("promosi_disc","disc_prc_2","double");
    $this->add_field("promosi_disc","disc_prc_3","double");


    $this->add_field("gl_report_groups","cid");
	...
	function create_zzz_supplier_need_update(){
 	...
 	function create_zzz_invoice_need_update(){
        $fields[]="invoice_number nvarchar(50)";
        $fields[]="tanggal datetime";
        $fields[]="gudang nvarchar(50)";                
        $this->create_table("zzz_invoice_need_update",$fields);		
	}
    function create_zzz_purchase_need_update(){
        $fields[]="invoice_no nvarchar(50)";
        $fields[]="tanggal datetime";
        $fields[]="gudang nvarchar(50)";                
        $this->create_table("zzz_purchase_need_update",$fields);		
	}
    function create_zzz_sales_order_need_update(){
        $fields[]="sales_order_number nvarchar(50)";
        $fields[]="tanggal datetime";
        $fields[]="gudang nvarchar(50)";                
        $this->create_table("zzz_sales_order_need_update",$fields);		
	}
...
function create_inventory_categories_sub(){
 ...
 function add_fields($table,$fields)
 {
    ...
    //if(""==$this->CI->sysvar->getvar($key) ){       
        ...
        $exist=false;
        $field_new="";
        for($iflds=0;$iflds<count($fields);$iflds++){            
            $exist=false;
            ...
            if(!$exist){
                add_field($table,$fields[$iflds]);
                //$field_new.=$fields[$iflds];
                //if($iflds<count($fields)-1)$field_new.=",";
            }
        }
        //$s="ALTER TABLE `$table` ADD COLUMN `$field_new` $type ; "; 
