...
function select_list(){return $this->customer_list();}
	...
	function shipto_add($data){           
		if($data['location_code']=="AUTO" || $data['location_code']==""){
			$data['location_code']=$this->shipto_next_number();
			$this->shipto_next_number(true);
		}
		return $this->db->insert("customer_shipto",$data);            
	}
	function shipto_next_number($add=false)
	{
		$key="customer shipto numbering";
		if($add){
		  	$this->sysvar->autonumber_inc($key);
		} else {			
			$no=$this->sysvar->autonumber($key,0,'!S~$00001');
			return $no;			
		}
	}
    ...
    function lookup(){        
        ...
        $lookup = $this->list_of_values->render(array(
        	"dlgBindId"=>"customers",
        	"dlgUrlQuery"=>base_url("index.php/customer/browse_data"),
       		'show_checkbox'=>false,
       		'show_check1'=>true,'check1_title'=>"Customer",'check1_field'=>'customer_number',
        	"dlgRetFunc"=>"			
				$('#customer_number').val(row.customer_number);
				$('#cust_code').val(row.customer_number);
				$('#cust_no').val(row.customer_number);
				$extra_ret_func
        	",
        	"dlgCols"=>array(
                array("fieldname"=>"customer_number","caption"=>"Kode","width"=>"80px"),
                array("fieldname"=>"company","caption"=>"Nama","width"=>"180px")
        	)
        ));
		return $lookup;     
    }	
