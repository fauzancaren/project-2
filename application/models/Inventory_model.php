...
class Inventory_model extends CI_Model {
    ...
    function save($data,$id=""){
        ...
        if(isset($data['cost_from_mfg'])){
            if($data['cost_from_mfg']=="")$data['cost_from_mfg']=0;
			$data['cost_from_mfg']=c_($data['cost_from_mfg']);
			if(isset($data['cost']) && $data['cost']==0)$data['cost']=$data['cost_from_mfg'];
		}
        if(!is_numeric($data['cost_from_mfg']))$data['cost_from_mfg']=0;
        if(isset($data['cost']))if(!is_numeric($data['cost']))$data['cost']=0;
        if(isset($data['retail']))if(!is_numeric($data['retail']))$data['retail']=0;
        if(!isset($data['disc_prc_1']))$data['disc_prc_1']=0;
...
//		$this->replicate_model->inventory($item_no);
    ...
    function quantity_in_stock($item_number){
        ...
        return $this->qry_kartu_stock_union_model->GetQtyStockGdg($item_number);
    }
	function quantity_in_stock_old($item_number)
	{
...
//        $data['qty_akhir']=$qty_akhir;
    ...
    function recalc_stock_gudang($item_number="",$tahun="",$bulan=""){
        ...
        $sql=$this->qry_kartu_stock_union_model->GetQtyStockGdgSql($item_number);
        $sql="select gudang,sum(qty) as qty,sum(qtyin) as qin,sum(qtyout) as qout 
        from tmp_kartu_stock_1 group by gudang";
