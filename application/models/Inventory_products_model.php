...
private $message="";
	...
	function valid_data($data){
		...
		if (isset($data['mu_qty'])){
			if($data['mu_qty']==''){
				...
				if(isset($data['cost']))$data['mu_price']=$data['cost'];
			}
		}
		...
		if(isset($data['cost'])){
			$cost=$data['cost'];
			$total_amount=floatval($data['quantity_received'])*floatval($cost);
    ...
    function update_header($id,$data){
        ...
        i.kode_lama,i.item_number as item_number_stock,ip.multi_unit
