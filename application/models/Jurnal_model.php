...
public $silent_mode=false;
	...
	function del_jurnal($gl_id){
		...
		if($q=$this->db->query("select date from gl_transactions where gl_id='$gl_id' limit 1")) {
			if($r=$q->row()){
				...
				if($this->periode_model->closed($r->date)){
					$this->message= "ERR_PERIOD: date: Periode ".$r->date." sudah ditutup ! ".$this->periode_model->closed($r->date);
					$success=false;
				}
			}
		}
		...
		if(!$success)$this->message="Error hapus jurnal mungkin periode sudah ditutup!";
		return $success;
	}
	function balance($gl_id) {
		if($this->db->query("select count(1) as cnt from gl_transactions where gl_id='$gl_id'")->row()->cnt==0) {
			...
			$z_amt=0;
			$s="select sum(debit)-sum(credit) as z_amt from gl_transactions where gl_id='$gl_id'";
			if($r= $this->db->query($s)->row()) {
				$z_amt=$r->z_amt;
			}
			if(abs($z_amt)<10)$z_amt=0;
			return $z_amt==0;
		}
	}
