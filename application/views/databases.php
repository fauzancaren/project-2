...
if(!isset($multi_company_type))$multi_company_type=0;
...
<div class="container " >
				...
				<div class="panel-body"  >
						...
						action="<?=base_url()?>index.php/company/open_database">
							...
							<?php 
							 if($multi_company){
								...
								if(!multi_company_single_db()) {
									...
									echo "<select onchange='on_select_db(this.value);return false;' class='form-control' id='company_code' name='company_code'>";
									...
									if($q=$this->db->select("company_code,company_name")
										...
										foreach($q->result() as $row){
											...
											echo "<option 
												value='$row->company_code' $selected >
							...
							<div class="form-group">
								<input class="btn btn-default" type="submit" value="Change Password" name='submit' style="height:30px">
	...
	<div class="row">
		<div class="panel panel-primary"  style="margin:5px;border:0px solid white">
		<div class='panel-body'>
		<img src="<?=base_url()?>images/logo_maxon.png" style="float:left;height:100px;width:150px;margin-right:20px">
...
function on_select_db(company_code){
	var gudang_list = $("#warehouse_code");
	gudang_list.find('option').remove().end();
	var _url=CI_ROOT+"company/warehouse/"+company_code;
	$.ajax({
  	 	url: _url,
  	 	success:function (result) {
			console.log(result);
			...
			for(i=0;i<data.length;i++){
				var r=data[i];
				gudang_list.append('<option value="'+r.location_number+'">'+r.location_number+'</option>').val(r.location_number);
			}
  	 	}
  	 });
}
