<div class="thumbnail box-gradient">
    ...
    if($closed=="0"){
        echo link_button('Closing', 'onClosing();return false;','edit');             
    } else {
        echo link_button('ReOpen', 'onUnClosing();return false;','edit');               
    }
...
<?php
		...
		$unpost_tr->fnc_edit="view_doc(row);return false";
...
//            echo form_open('periode/update','id=\'myform\' name=\'myform\' class=\'form-horizontal\' role=form');
	...
	function onUnClosing(){
        loading();
		var period=$("#startdate").val();
		var url=CI_ROOT+"periode/unclosing/"+period;
	...
	function onClosing(){
        loading();
		var period=$("#period").val();
		var url=CI_ROOT+"periode/closing/"+period;
		$.ajax({url: url,type: "GET",
		success: function(result)
			{
				loading();
	...
	function view_doc(row){
		console.log(row);
		add_tab_parent("View: "+row.nomor_bukti,CI_ROOT+"gl/view_doc/"+row.nomor_bukti);
	}
</script>
