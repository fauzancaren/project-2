...
<form id='myform' method='post' action='<?=base_url()?>index.php/receive_po/proses'>
   <table class='table2' width="100%">
	...
	<div class='alert alert-info' style="font-size:small">
		...
		<p>Karena keterbatasan PHP post usahakan diterima jangan melewati <b>100 baris</b>, pecah menjadi dua kali penerimaan.</p>
	</div>
</div>
	...
	function po_open_selected(){
		...
		if (row){
			...
			$('#dlgSelectFaktur').dialog('close');
	 		$("#divItem").fadeIn("slow");
			reload_po(nomor);
			var t=setTimeout(function(){reload_po(nomor);},3000);
		} else {
			log_err("Pilih salah satu nomor purchase order !");
		}
	}
	function reload_po(nomor){
