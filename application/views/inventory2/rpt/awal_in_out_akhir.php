     ...
     <tr>
 		...
 		<tbody>
				...
				left join (select k.item_number,sum(k.qty_masuk)-sum(k.qty_keluar) as qty_awal,
		 		        ...
		 		        sum(k.qty_masuk-k.qty_keluar) as m_qty_awal,
						sum((k.qty_masuk-k.qty_keluar)*k.price) as m_amt_awal   
				...
				left join (select k.item_number, sum(k.qty_masuk) as qty_in, 
						sum(k.qty_keluar) as qty_out,
		 		        ...
		 		        sum(k.qty_masuk-k.qty_keluar) as m_qty,
						sum((k.qty_masuk-k.qty_keluar)*k.price) as m_amt		 		           
		                from qry_kartustock_union k
				...
				$CI->qry_kartu_stock_union_model->getSql(
					"sum","",$date1,"",$kel
				);
				$CI->qry_kartu_stock_union_model->getSql(
					"list",$date1,$date2,"",$kel
				); 
				$sql="select c.item_number,i.description,i.unit_of_measure,
					sa.qty_awal,
					sum(c.qtyin) as qty_in,sum(c.qtyout) as qty_out,
					sa.qty_awal+sum(c.qtyin-c.qtyout) as qty_akhir,
					sa.amount_awal,
					sum(c.amountin) as amt_in,sum(c.amountout) as amt_out,
					sa.amount_awal+sum(c.amountin-c.amountout) as amount_akhir,
					sum(c.qtyin-c.qtyout) as m_qty
					from tmp_kartu_stock c 
					left join (
						select item_number,sum(qtyin-qtyout) as qty_awal,
						sum(amountin-amountout) as amount_awal
						from tmp_kartu_stock 
						where doctype='SALDO'
						group by item_number
					) sa on sa.item_number=c.item_number 
					left join inventory i on i.item_number=c.item_number
					where c.doctype<>'SALDO'
					group by c.item_number,i.description,i.unit_of_measure,sa.qty_awal,sa.amount_awal
