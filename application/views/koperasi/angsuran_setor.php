...
<div class="easyui-tabs" style="width:auto;height:auto;min-height:300px">
	...
	<form id="frmLoan"  method="post">
	   ...
	   <table class='table'>
			<tr><td>Nomor Bukti</td>
				<td>
					...
					if($mode=='view'){
						...
						echo "<input type='hidden' id='payment_no' name='payment_no' value='$payment_no'>";
					} else { 
		   ...
		   <tr>
				<td>Angsuran Ke</td><td><?=form_input('bulan_ke',$bulan_ke,"id=bulan_ke");?></td>
				<td>Jumlah Tagihan</td><td><?=form_input('tagihan',$tagihan,"id=tagihan");?></td>
		   </tr>
			<tr>
...
<script type="text/javascript">
    function save_this(){
		...
		url='<?=base_url()?>index.php/koperasi/angsuran_setor/save';
			$('#frmLoan').form('submit',{
				url: url,
				onSubmit: function(){
				...
				success: function(result){
					...
					if (result.success){
						$('#payment_no').val(result.payment_no);
						$('#mode').val('view');
	...
	function select_pinjaman(){
		lookup1({id:"kop_pinjaman", 
			url:CI_ROOT+"lookup/sys_sql_data/lookup.kop.pinjaman",
			fields: [[
	...
	function cari_no_pinjaman(){
		var no_pinjaman=$("#no_pinjaman").val();
		if(no_pinjaman==""){
			log_msg("Nomor pinjaman tidak ada !");
			return false;
		}
		var url=CI_ROOT+'koperasi/pinjaman/info/'+no_pinjaman;
				$.post(url,null,function(result){
					if (result.success){
						//log_msg("Nomor berhasil dihapus.");
						$("#bulan_ke").val(result.bulan_ke);
						$("#tagihan").val(result.tagihan);
						$("#tanggal_tagih").val(result.tanggal_tagih);
