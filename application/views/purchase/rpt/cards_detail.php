     ...
     <tr>
 		...
 		<thead>
 			<tr>
 				...
 				<td>Sistim</td><td>Termin</td><td>Tgl. JTempo</td>
 			</tr>
 		</thead>
 		<tbody>
     	<?php
		...
		foreach($qcard->result() as $rcard){
			...
			$tgl_jth_tempo="";
				$sistim="";
			if($q=$this->db->query("select type_of_invoice,terms 
				from purchase_order where purchase_order_number='$rcard->NoBukti'")){
				if($r=$q->row()){
					$sistim=$r->type_of_invoice;
					$termin=$r->terms;
					$tgl_jth_tempo=due_date($rcard->Tanggal,$termin);
				}
			}
