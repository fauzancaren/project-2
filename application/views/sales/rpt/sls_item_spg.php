<?php 
	$CI =& get_instance();
	$data['caption']='PENJUALAN PER ITEM SPG';
	if(!$CI->input->post('cmdPrint')){
		...
		$data['label1']='Spg';
		$data['text1']='';
        $data['key1']="employee_id";
        $data['fields1'][]=array("salesman","80","Salesman");
        $data['fields1'][]=array("salesman","180","Salesman");
        $data['ctr1']='lookup/query/salesman';
		$data['rpt_controller']="sales/rpt/$id";
		$CI->template->display_form_input('criteria',$data,'');
	} else {
		$date_from=$CI->input->post("txtDateFrom");
		$date_to=$CI->input->post("txtDateTo");
		$employee_id=$CI->input->post("text1");
		$salesman="";
		
		$sql="select il.employee_id,
		...
		if($employee_id!="")$sql.=" and il.employee_id='$employee_id'";
		...
		$sql.=" group by il.employee_id,il.item_number,il.description";		
		
		$data['content']=browse_select(	
			array('sql'=>$sql,'show_action'=>false,
				"group_by"=>array("employee_id"),
				"fields_sum"=>array("z_qty","z_amount"),
				"hidden"=>array("employee_id")
			)
		);
		 $data['header']=company_header();
		 $data['criteria']="Date From: $date_from - $date_to, Spg: $employee_id";
