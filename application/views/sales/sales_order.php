<div class="thumbnail box-gradient" id='divToolbar'>
	<?php
		...
		echo link_button('Print2', 'print_bo()','print');
//		echo link_button('Add','','add','false',base_url().'index.php/sales_order/add');		
		echo link_button('Delete','hapus();return false','cut');		
...
<div class='col'>
	<div class="easyui-tabs" >
		...
		<div id="divPay" title="Pembayaran (Deposit)" >
			<?php 
				$invoice_number=$sales_order_number;
...
<script type="text/javascript">
	...
	function hitung_jumlah(){
        ...
        if(disc_prc>1) 	disc_prc=disc_prc/100;
	    ...
	    $.ajax({
            ...
            success: function(msg){
                ...
                $('#disc_total_amount_1').val(obj.disc_amount_1);
                $('#tax').val(obj.tax);
            },
  	...
  	function print_bo(){
		...
		so=$('#sales_order_number').val(); 
		window.open("<?=base_url().'index.php/sales_order/print_bo/'?>"+so,"new");  	
  	}
	function approve(){
		if(!allow_approve) {
			log_msg(ERR_ACCESS_MODULE);
	...
	function hapus(){
		var no=$("#sales_order_number").val();
		if(no==""){
			log_msg("Kode tidak ada !"); return false;
		}
		$.messager.confirm('Confirm','Are you sure you want to remove this?',function(r){
		if (r){
			url='<?=base_url()?>index.php/sales_order/delete_json/'+no;
			$.post(url,{sales_order_number:no},
			function(result){
				if (result.success){
					remmove_tab_parent();
				} else {
