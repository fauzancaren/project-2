<div id='dgItem' class="easyui-dialog" style="width:700px;height:400px;
    ...
    <form id="frmItemInv" method='post' >
        ...
        <tr>
            <td>Kode Barang</td>
            <td colspan=4><input onblur='find();return false;' id="item_number" style='width:150px' 
                name="item_number"  >
        ...
        <tr>
            <td>Nama Barang</td>
            <td colspan=4><input id="description" name="description" style='width:350px'></td>
        </tr>
        <tr>
             <td>Qty</td>
			 <td colspan=3><input id="quantity"  style='width:80px'  name="quantity" onblur="hitung()">
        ...
        <tr>
            <td>Disc%1 </td><td><input id="discount" name="discount"  style='width:80px'   
				onblur="hitung();" class="easyui-validatebox" validType="numeric"> 
				Disc1 Rp  <input id="disc_amount_1" name="disc_amount_1"  style='width:100px'   
				onblur="hitung()" class="easyui-validatebox" validType="numeric">
			</td>
        </tr>
		<tr>
			<td>Disc%2 </td><td><input id="disc_2" name="disc_2"  style='width:80px'   
				onblur="hitung()" class="easyui-validatebox" validType="numeric"> 
				Disc3 Rp <input id="disc_amount_2" name="disc_amount_2"  style='width:100px'   
				onblur="hitung()" class="easyui-validatebox" validType="numeric">
			</td>
		</tr>
		<tr>
			<td>Disc%3 </td><td><input id="disc_3" name="disc_3"  style='width:80px'   
				onblur="hitung()" class="easyui-validatebox" validType="numeric">
			 	Disc3 Rp <input id="disc_amount_3" name="disc_amount_3"  style='width:100px'   
				onblur="hitung()" class="easyui-validatebox" validType="numeric">
			</td>
		</tr>
        <tr>
            <td><b>Jumlah</b></td><td><input id="amount" name="amount"  
		...
		<tr><td></td></tr>
        <tr><td>Qty Saldo:</td><td><span id='qty_gudang'>*select item first</span></td>
        </table>
    </form>
</div>  
<div id='btnItem'>
...
function clear_input(){
	...
	$('#item_number').val('');
	$('#discount').val('0');$('#disc_amount_1').val('0');
	$('#disc_2').val('0');$('#disc_amount_2').val('0');
	$('#disc_3').val('0');$('#disc_amount_3').val('0');
	$('#unit').val('');
	$('#description').val('');
	$('#line_number').val('');
	$('#quantity').val(1);
...
function editItem(){
	...
	if (row){
		...
		$('#unit').val(row.unit);
		$('#price').val(row.price);
		$('#discount').val(row.discount);$('#disc_amount_1').val(row.discount_amount);
		$('#disc_2').val(row.disc_2);$('#disc_amount_2').val(row.disc_amount_2);
		$('#disc_3').val(row.disc_3);$('#disc_amount_3').val(row.disc_amount_3);
...
function hitung(){
	...
	$('#discount').val(disc_1);
	disc_amount_1=c_($('#disc_amount_1').val());
	if(disc_1>0){
		disc_amount_1=gross*disc_1;	
		$('#disc_amount_1').val(disc_amount_1);
	}
...
function find(){
	    ...
	    $.ajax({
			...
			success: function(msg){
				...
				var qty="<i>";
				var i=0;
				for(i=0;i<obj.qty_gudang.length;i++){
					qty=qty+''+obj.qty_gudang[i].warehouse_code + ': ' + obj.qty_gudang[i].quantity+",";
				}
				qty=qty + "</i>";
				$("#qty_gudang").html(qty);
