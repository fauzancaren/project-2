...
<script language="JavaScript">
    ...
    function run_timer_inbox(){
        timer1();
        _timer1=setTimeout(function(){run_timer_inbox()}, 60000*5);    //1menit
    ...
    function run_timer_jurnal(){
        ...
        trun=setTimeout(function(){run_timer_jurnal()}, 60000*10);
        $.ajax({
                type: "GET",
    ...
    function run_timer_replicate() {
        ...
        trun=setTimeout(function(){run_timer_replicate()}, 60000*30);
        $.ajax({
                type: "GET",
    ...
    function run_timer_recalc_inventory() {
        ...
        trun=setTimeout(function(){run_timer_recalc_inventory()}, 60000*6);
        $.ajax({
                type: "GET",
    ...
    function run_timer_notify() {
        ...
        trun=setTimeout(function(){run_timer_notify()}, 60000*15);
        $.ajax({
                type: "GET",
    ...
    function run_timer_recalc_master() {
        ...
        trun=setTimeout(function(){run_timer_recalc_master()}, 60000*15);
        $.ajax({
                type: "GET",
    ...
    function run_timer_purchase() {
        _request_count++;
        if(_request_count>15){
            return;
        }
        $('#msg-box-wrap').html("Run: recalc_master");     
        trun=setTimeout(function(){run_timer_purchase()}, 60000*15);
        $.ajax({
            type: "GET",
            url: '<?=base_url();?>index.php/purchase_order/recalc_run',
            contentType: 'application/json; charset=utf-8',
            success: function(msg){
                if(msg!=''){
                    $("#msg-box-wrap").html('run_timer_purchase()');                    	
                    _request_count--;
                }
            }
        });
    }    
    function run_timer_sales() {
        _request_count++;
        if(_request_count>15){
            return;
        }
        $('#msg-box-wrap').html("Run: recalc_master");     
        trun=setTimeout(function(){run_timer_sales()}, 60000*15);
        $.ajax({
            type: "GET",
            url: '<?=base_url();?>index.php/sales_order/recalc_run',
            contentType: 'application/json; charset=utf-8',
            success: function(msg){
                if(msg!=''){
                    $("#msg-box-wrap").html('run_timer_sales()');                    	
                    _request_count--;
                }
            }
        });
        $.ajax({
            type: "GET",
            url: '<?=base_url();?>index.php/invoice/recalc_run',
            contentType: 'application/json; charset=utf-8',
            success: function(msg){
                if(msg!=''){
                    $("#msg-box-wrap").html('run_timer_sales()');                    	
