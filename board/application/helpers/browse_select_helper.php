				...
				while ( ! $eof and $k<count($rows) and $new_val==$old_val ) {
					...
					for($i=0;$i<$count;$i++){
						...
						if(!$lhide){
							...
							if($type[$i]==5 || $type[$i]=='real' || $type[$i]=='5' || $type[$i]=="double" || $type[$i]=="int" || $type[$i]==3){
								$val=$row[$fld];
//								if($val>0 and $val<1){
...
//						$CI->table->add_row("<strong>Sub total  $group_by1 : $old_val Rp. ".$sub_ttl[$fields_sum[0]]."</strong>");
						...
						for($ik=0;$ik<$count;$ik++){
							...
							$s2[$ik]="";							
							if(isset($fields_sum_value[$fld])){
								$s2[$ik]='<div align="right"><strong>'.number_format($sub_ttl[$fld],2).'</strong></div>';
							} else {
		...
		for($i=0;$i<$count;$i++){
			...
			if(isset($fields_sum_value[$fld])){				
				$s[$i]='<div align="right"><strong>'.number_format($fields_sum_value[$fld],2).'</strong></div>';
			} else {
				if(!$lhide){
					$s[$i]="";
