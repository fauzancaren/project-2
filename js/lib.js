        ...
        if ($('#tt').tabs('exists', title)){ 
			...
			$.ajax({ type: "GET", url: url, data: param,
				success: function(result){
					var img=CI_BASE+"images/loading.gif";
					var content = "<div id='tab"+title+"'>"+result+"</div>"; 					
					$('#tt').tabs('add',{
						title: title,
						content: content,
						closable: true
...
$(document).ready(function(){
	...
	function lookup3(param){
		...
		var dlg="dialog_"+id;
		var table="table_"+id;
		var result_func=param.result_func;

		var themes="" + param.themes;
		if(themes=="undefined")themes="";
		var output_script = param.output_script;

		if(output_script==undefined || themes!="admin"){
			output_script="body";
		} else {
		...
		if(! $("#"+dlg).html() ){
			var dlgClass="";
			var tbl="<div id='"+dlg+"' class='modal fade' role='dialog' >";
			tbl = tbl + "<div class='modal-dialog' role='document' style='width:90%;height:80%;'>";
			tbl = tbl + "<div class='modal-content' >";
			tbl = tbl + "<div class='modal-header'>";
			tbl = tbl + "<h3 class='modal-title' id='dlg"+dlg+"Label'>"+dlg+"</h3>";
			tbl = tbl + "</div>";
			tbl = tbl + "<div class='modal-body' style='overflow:scroll;height:80%'>";  

			tbl = tbl + "<table id='"+table+"' width=100% class='display table'  > \r "; 
			tbl = tbl +"<thead><tr>";
			for(i=0;i<fields[0].length;i++){
				fld=fields[0][i].field;
				tbl = tbl + "<th>" + fld + "</th>";
			}
			tbl = tbl + "<th>Cmd</th></tr></thead>";
			
			tbl = tbl + "</table>  \r ";

			tbl = tbl + "</div>"; //modal-body 
			tbl = tbl + "<div class='modal-footer'>";
			tbl = tbl + "    <button type='button' class='btn-sb btn-danger-sb' data-dismiss='modal'> Close </button>";
			tbl = tbl + "</div>";

			tbl = tbl + "</div>"; //modal-conten

			tbl = tbl + "</div>";	 

			tbl = tbl + "</div>";  

			tbl = tbl + "<script> \r"+ "var r=null; \r " + result2 +  "\r" 

			 
				tbl = tbl + "function btnClick_"+table+"(){return '<button>Select</button>'; };";
				tbl = tbl + "$(document).ready(function() { \r";
				tbl = tbl + " var dataTable"+dlg+"=null; var cols"+dlg+"=[";
				...
				tbl = tbl + " if(dataTable"+dlg+"==null) {  \r" +
							" 	dataTable"+dlg+" = $('#"+table+"').DataTable({  \r" + 
									" serverSide: true, 'select':true, \r" +                        
									" ajax: {'type':'POST','url': '" + url +  "'}, \r";
				tbl = tbl + "	columns: cols"+dlg+", \r";  
				tbl = tbl + "	columnDefs:[{data:null,render: btnClick_"+table+",targets:"+i+"}] \r"
				tbl = tbl +	"}); \r";     
				tbl = tbl + "$('#"+table+" tbody').on( 'click', 'td button', function () { \r " ;
				tbl= tbl + "	var data = dataTable"+dlg+".row($(this).closest('tr')).data() ; \r ";
				tbl = tbl + "  	" + result_func + "(data);";			
				tbl = tbl + "} ); \r";

				tbl = tbl + "} else { \r dataTable.draw(); \r };";

				tbl = tbl + "});"; //document ready

			tbl = tbl + "<\/script> \r";

			$(output_script).append( $( tbl ) );
		}
