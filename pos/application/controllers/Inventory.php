...
///			$this->template->display_browse2($data);
	...
	function find($item_number='',$cust_type='',$min_qty=0){
		...
		supplier_number,item_picture,item_picture2,item_picture3,item_picture4,
		create_date,weight,allowchangeprice,allowchangedisc 
		...
		if($data){
			...
			$data['disc_amt_1']=$disc_prc['disc_amt_1'];
			$data['disc_amt_2']=$disc_prc['disc_amt_2'];
			$data['disc_amt_3']=$disc_prc['disc_amt_3'];
...
/* create_graph($konfigurasi_grafik, $data, $tipe_grafik, $judul_pd_grafik, $nama_berkas) */		
   ...
   function qty_gudang2($item_number){
	...
	function promo_disc_item($item_number){
		...
		$disc1amt=0;	$disc2amt=0;	$disc3amt=0;
		$promosi_code="";
		...
		if($disc1==0) {
			$sql="select promosi_code,disc_prc_1,disc_prc_2,disc_prc_3 
				from promosi_item 
		...
		if($disc1==0){
			$s="select p.promosi_code,p.tipe,p.nilai,p.nilai2,p.nilai3
			from promosi_item i 
			join promosi_disc p on p.promosi_code=i.promosi_code where '".date('Y-m-d H:i:s')."' 
			between p.date_from and p.date_to  and p.category='1' and p.flag1='0' 
			and i.item_number='$item_number'";	
			// and p.tipe='2'
			if($q=$this->db->query($s)){
				if($r=$q->row()){
					$promosi_code=$r->promosi_code.'~3';
					if($r->tipe=='2'){
						$disc1=$r->nilai;	$disc1amt=0;
						$disc2=$r->nilai2;	$disc2amt=0;
						$disc3=$r->nilai3;	$disc3amt=0;
	
					} else {
						$disc1amt=$r->nilai;	$disc1=0;
						$disc2amt=$r->nilai2;	$disc2=0;
						$disc3amt=$r->nilai3;	$disc3=0;
		...
		if($disc1==0){
			...
			if($category!=""){
				$s="select p.promosi_code,p.nilai,p.nilai2,p.nilai3
				from promosi_item i 
				join promosi_disc p on p.promosi_code=i.promosi_code where '".date('Y-m-d H:i:s')."' 
				between p.date_from and p.date_to and p.category='1'  and p.tipe='2'  and p.flag1='0' 
				and i.item_number='$category' and i.item_type='category'";	
				if($q=$this->db->query($s)){
					if($r=$q->row()){
						$disc1=$r->nilai;
						$disc2=$r->nilai2;
						$disc3=$r->nilai3;
						$promosi_code=$r->promosi_code.'~4';
					}	
				}					
			}
			if($supplier!="" && $disc1==0){
				...
				join promosi_disc p on p.promosi_code=i.promosi_code where '".date('Y-m-d H:i:s')."' 
				between p.date_from and p.date_to  and p.category='1' and p.tipe='2' and p.flag1='0' 
				and i.item_number='$supplier' and i.item_type='supplier'";	
				if($q=$this->db->query($s)){
					if($r=$q->row()){
						$disc1=$r->nilai;
						$disc2=$r->nilai2;
						$disc3=$r->nilai3;
						$promosi_code=$r->promosi_code.'~4 supplier';
					}	
				}					
			}
			if($manufacturer!="" and $disc1==0){
				...
				join promosi_disc p on p.promosi_code=i.promosi_code where '".date('Y-m-d H:i:s')."' 
				between p.date_from and p.date_to  and p.category='1' and p.tipe='2' and p.flag1='0' 
				and i.item_number='$manufacturer' and i.item_type='merk'";	
				if($q=$this->db->query($s)){
					if($r=$q->row()){
						$disc1=$r->nilai;
						$disc2=$r->nilai2;
						$disc3=$r->nilai3;
						$promosi_code=$r->promosi_code.'~4 merk';
					}	
				}					
			}
		...
		$retval['disc_prc_1']=$disc1;	$retval['disc_amt_1']=$disc1amt;
		$retval['disc_prc_2']=$disc2;	$retval['disc_amt_2']=$disc2amt;
		$retval['disc_prc_3']=$disc3;	$retval['disc_amt_3']=$disc3amt;
		$retval['promosi_code']=$promosi_code;
		//var_dump($retval);
...
//			$pdf->AddPage();
	...
	function recalc_ex($item_number="",$tahun="",$bulan=""){
		$msg="";	
