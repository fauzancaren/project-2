...
class Kas_keluar extends CI_Controller {
    ...
    private $sql="";
    private $file_view='sales/kas_keluar';
    private $primary_key='no_bukti';
    private $controller='kas_keluar';
	...
	{
		$key="Kas Kasir Keluar Numbering";
		if($add){
		  	$this->sysvar->autonumber_inc($key);
		} else {			
			$no=$this->sysvar->autonumber($key,0,'!KKK~$00001');
			for($i=0;$i<100;$i++){			
				$no=$this->sysvar->autonumber($key,0,'!KKK~$00001');
	...
	function update()
	{
		 ...
		 if ($this->form_validation->run()=== TRUE){
			...
			unset($data['id']);
            $data['jumlah']=-1*abs($data['jumlah']);
			$this->kasir_kasir_model->update($id,$data);
	...
	{
        $data['caption']='DAFTAR KAS KELUAR';
...
//		$no=$this->input->get('sid_nomor');
    ...
    function print_bukti($nomor){
		...
		$data['content']=load_view('sales/rpt/print_kas_keluar',$data);
