...
class User extends CI_Controller {
	...
	function __construct()
	{
		...
		$this->sql="select user_id,username,cid,branch_code,supervisor,nip,i.cid,
			i.address,i.userlevel,i.active 
			from user i";
	}
	function set_defaults($record=NULL){
		$data=$this->user_model->set_defaults($record);           
        return $data;
	}
	function index()
	{	
		...
		if($cmd=="exist"){
			...
			if($row=$this->user_model->get_by_id($username)->row()){
				$data['success']=false;
				$data['message']="Username [$username] tidak boleh dipakai !! Silahkan ganti userid/username lain.";
			} else {
				$data['success']=true;
				$data['message']='Username tersedia dan boleh dipakai';				
			}
   ...
   function browse($offset=0,$limit=50,$order_column='sales_order_number',$order_type='asc'){
        ...
        $data['fields_caption']=array('User ID','Nama User','Kelompok','Cabang',
		'Supervisor','NIP','CID');
        $data['fields']=array('user_id','username','cid','branch_code','supervisor',
		'nip','cid');
	...
	function save_json(){
		$data=null_text_clear($this->input->post());
		$app="";
		if(isset($data['app'])) {
			$app=$data['app'];
			unset($data['app']);
		...
		if(isset($data['mode'])){
			...
			$mode="add";
			if($r=$this->db->query("select count(1) as cnt from user 
				where user_id='$id'")->row()){
				if($r->cnt>0)	$mode="view";
			}
		}
	...
	{   
		 ...
		 $this->_set_rules();
		  ...
		  $data_input = null_text_clear($data_input);
		
		 if ($this->form_validation->run()=== TRUE){
			$mode=$data_input["mode"];
			unset($data_input["mode"]);
			unset($data_input['path_image']);
			if($data_input['flag1']=="true"){
				$data_input['flag1']='1';
			} else {
				$data_input['flag1']='0';
			}
			if($mode=="view"){
				...
				$ok=$this->user_model->update($id,$data_input);			
			} else {
				$ok=$this->user_model->save($data_input);
			}
		} else {
			$ok=false;
		}	
	...
	function login($username="", $password="")
	{
		...
		if($username=="")$username=$this->input->post("user_id");
		if($password=="")$password=$this->input->post("password");
	...
	function accepted(){
		...
		$uid=$data['uid'];
		$pwd=$data['pwd'];
		$message='Unknown Error UserId or Password !';
		$success=false;
		$this->load->model('user_model');
		if($q=$this->user_model->get_by_id($uid)){
			if($r=$q->row()){
				if($r->user_id==$uid && $r->password==$pwd){
					$success=true;
					$message="Success";
				}
			}
