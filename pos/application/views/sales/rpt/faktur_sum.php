     ...
     <tr>
     	<td colspan="8">
	     		...
	     		<thead>
	     			...
	     			<td>Paid</td><td>Pelanggan</td><td>Salesman</td><td>Termin</td>
                    <td>So#</td></tr>
	     		</thead>
	     		<tbody>
     			<?
     			$sql="select i.*,c.customer_number,c.company from invoice i 
				...
				$z_db_amount=0;		$z_saldo=0;
                 foreach($rst_so->result() as $row){
                    ...
                    if($outlet==$warehouse_code || $outlet==''){
                        ...
                        $tbl.="<td align='right'>".number_format($row->inv_amount)."</td>";
                        $tbl.="<td>".$row->paid."</td>";
                        ...
                        $tbl.="</tr>";
    					$z_amount=$z_amount+$row->inv_amount;
					}
               };
