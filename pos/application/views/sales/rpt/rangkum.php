<?php
    ...
    if(user_flag2() && $date1<date("Y-m-d")){
        echo "<p>Tanggal awal dibatasi hanya hari ini !</p>"; exit;
    }
...
$s="select count(1) as z_cnt,sum(if(coalesce(amount,0)=0,inv_amount,amount)) as z_amt, 
...
if($outlet!="")$s.=" and warehouse_code='$outlet' ";
if($salesman!="")$s.=" and salesman='$salesman' ";


if($row=$CI->db->query($s)->row()){
        $nota_cnt=$row->z_cnt;
        $amt_nota=$row->z_amt;
...
$s="select count(1) as z_cnt,sum(amount) as z_amt,sum(inv_amount) as z_amt_2 
from invoice where invoice_type='I' 
and invoice_date between  '$date1' and '$date2' and (paid is null or paid=0) ";
if($outlet!="")$s.=" and warehouse_code='$outlet' ";
if($salesman!="")$s.=" and salesman='$salesman' ";
if($row=$this->db->query($s)->row()){
...
$s="select sum(il.amount) as z_amt, sum(il.discount_amount) as z_disc1, 
sum(il.disc_amount_2) as z_disc2,sum(il.disc_amount_3) as z_disc3,
sum(il.disc_amount_ex) as z_disc_rp,sum(inv_amount) as z_amt_2 
...
if($outlet!="")$s.=" and warehouse_code='$outlet' ";
if($salesman!="")$s.=" and salesman='$salesman' ";
$s.="group by p.how_paid, p.credit_card_type";
$pays=$this->db->query($s);

$s="select sum(jumlah) as z_amt 
from kas_kasir where tanggal between '$date1' and '$date2' 
and jumlah>0 ";
if($outlet!="")$s.=" and warehouse_code='$outlet' ";
if($salesman!="")$s.=" and kasir='$salesman' ";

$kas_in=$this->db->query($s)->row()->z_amt;
...
$kas_out=$this->db->query($s)->row()->z_amt;
