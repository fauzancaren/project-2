    ...
    <div class="thumbnail">
        ...
        <table class='table'>
            ...
            <tr><td colspan=4>
                ...
                <?=link_button("Transfer","dlgPaySplit_Tran()","sum");?>                
...
<script type='text/javascript' language="JavaScript">   
    ...
    function dlgPaySplit_Gopay_Submit(){
        ...
        dlgPaySplit_Addrow('GOPAY',gopay_amount,gopay_no);
        $("dlgGopay").dialog("close");
        
		calc_ttl_bayar_split();        
    	
    }
    function dlgPaySplit_Tran(){
    	calc_ttl_bayar_split();
    	var ttl_sisa_split=cval("#dlgPaySplit_Sisa");
    	...
    	$("#dlgTran_owner").val("");
    	$("#dlgTran_ref").val("");
        $("#dlgTran_amount").val(formatNumber(ttl_sisa_split));
        $("#dlgTran").dialog("open").dialog('setTitle','Info Transfer');
        
    }
    function dlgTran_Cancel(){
        $("#dlgTran").dialog("close");
    }
    function dlgTran_Submit(){
    	var amount=cval("#dlgTran_amount");
        var ref=$("#dlgTran_ref").val();
        var bank=$("#dlgTran_rek").val();
        var owner=$("#dlgTran_owner").val();
        if(amount==0){
        	log_err("Jumlah Transfer belum diisi !"); return false;
        }
        dlgPaySplit_Addrow('TRANSFER',amount,ref,bank,owner);
        $("#dlgTran").dialog("close");
        
		calc_ttl_bayar_split();        
    	
    }
    function dlgPaySplit_Addrow(jenis,amount,ref,rekening,owner){
    	if(rekening=="undefined")rekening="";
