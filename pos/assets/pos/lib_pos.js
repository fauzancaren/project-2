var next_row=0,tableData=null,selected_row=0,selected_line_id=0,button_mode="btn_qty",xqty='';
...
$(document).ready(function(){
	...
	function lock_input(){
		$("#item_disc_prc").removeAttr("readonly");
		$("#item_disc_prc_2").removeAttr("readonly");
		$("#item_disc_prc_3").removeAttr("readonly");
		$("#item_disc_amt").removeAttr("readonly");
		$("#item_disc_amt_2").removeAttr("readonly");
		$("#item_disc_amt_3").removeAttr("readonly");
		$("#item_price").removeAttr("readonly");
		$("#disc_nota_prc").removeAttr("readonly");
		$("#disc_nota_rp").removeAttr("readonly");
		...
		if(caption="Del"){
			...
			$.messager.confirm('Confirm','Are you sure delete row_id ['+id+'] ?',function(r){
				if (r){
					...
					if(id>0){
						loading();
						var url=base_url+'index.php/invoice/delete_item/'+id;
	...
	function get_header_nota(){
		...
		if(payment_terms=="")payment_terms="CASH";

		salesman=$("#salesman").val();
		if(salesman=="")salesman="OFFICE";
		...
		"payment_terms":payment_terms,"salesman":salesman};
...
function add_row_sales(){
    ...
    qty_saldo=c_($("#qty_saldo").html());
    qty_min=c_($("#qty_min").html());
    
    description=$("#item_nama_barang").val();		item_disc_prc_2=$("#item_disc_prc_2").val();
    qty=c_($("#qty").val());							item_disc_amt_2=$("#item_disc_amt_2").val();
    ...
    if(is_gabung_item && !lEditRow){
		var td=get_row(item_number);
		if(td!=null){
			qty=c_(qty)+c_(td[C_QTY]);
			m_qty=c_(m_qty)+c_(td[C_M_QTY]);
			item_disc_amt=c_(item_disc_amt)+c_(td[C_DISC_AMT]);
			$("#line").val(c_(td[C_ID]));
		}
...
function add_row_data(ar){
	...
	for(i=0;i<ar.length;i++){
		...
		add_row(a.no_urut,a.item_number,a.description,a.quantity,a.unit,
			...
			a.mu_qty,a.multi_unit,a.mu_harga,a.coa1amt,a.employee_id);
	}
}
...
function calc_input(){
    ...
    qty=$("#qty").val(); if(qty==''||qty=='NaN')qty=1;
    
    item_price=$("#item_price").val();
    item_price=item_price.replace(/,/g,"");
	if(item_price==''||item_price=='NaN')item_price=0;
    ...
    if(item_disc_amt==''||item_disc_amt=='NaN')item_disc_amt=0;
    ...
    if(item_disc_prc_2==''||item_disc_prc_2=='NaN')item_disc_prc_2=0;
    if(item_disc_prc_2>1){
    	item_disc_prc_2=item_disc_prc_2/100;
    }
    item_disc_amt_2=item_price*item_disc_prc_2;

    if(item_disc_amt_2==''||item_disc_amt_2=='NaN')item_disc_amt_2=0;
    ...
    if(item_disc_prc_3==''||item_disc_prc_2=='NaN')item_disc_prc_3=0;
    if(item_disc_prc_3>1){
    	item_disc_prc_3=item_disc_prc_3/100;
    }
    item_disc_amt_3=item_price*item_disc_prc_3;
    if(item_disc_amt_3==''||item_disc_amt_3=='NaN')item_disc_amt_3=0;
    ...
    total=total-item_disc_amt-item_disc_amt_2-item_disc_amt_3-item_disc_amt_ex;
	...
	function find_barcode(){
		...
		if(lEditRow)return false;
		//$("#barcode").val("");
		$("#qty").val("1");
		loading();
		$.ajax({
			type: "GET",
			url: url_item_find+barcode+'?cust_no='+cust,
			success: function(msg){
				loading_close();
				var result = eval('('+msg+')');
				if(result.success){
    ...
    function result_set(result){
        ...
        $("#item_disc_amt").val(result.disc_amt_1);
        $("#item_disc_amt_2").val(result.disc_amt_2);
        $("#item_disc_amt_3").val(result.disc_amt_3);
		...
		if(result.allowchangeprice=="0"){
			$("#item_price").attr("readonly",true);
			$("#m_price").attr("readonly",true);
		} else {
			$("#item_price").removeAttr("readonly");
			$("#m_price").removeAttr("readonly");
		}
		if(result.allowchangedisc=="0"){
			$("#item_disc_prc").attr("readonly",true);
			$("#item_disc_prc_2").attr("readonly",true);
			$("#item_disc_prc_3").attr("readonly",true);
			$("#item_disc_amt").attr("readonly",true);
			$("#item_disc_amt_2").attr("readonly",true);
			$("#item_disc_amt_3").attr("readonly",true);
	...
	function reports(){
		window.open(base_url+'index.php/reports','_blank');
	}
    function submit_session(){
        _url=CI_BASE+'index.php/user/session';        
        ...
        $.ajax({
                ...
                success: function(msg){
                    console.log(msg);
                    window.open(CI_BASE+"index.php","_self");
                }
        });
    }
	...
	function edit_row(){
		...
		//$("#disc_amount_ex").attr("readonly",true);
...
function add_row(next_row,item_number,description,qty,unit,retail, 
	...
	m_qty,m_unit,m_price,m_komisi,m_employee_id){
			
	var line_id=$("#line").val();

	if(next_row==null)next_row=0;

	if(qty==null)qty=1;
	if(item_disc_prc==null)item_disc_prc=0;
	if(item_disc_amt==null)item_disc_amt=0;
	if(item_disc_prc_2==null)item_disc_prc_2=0;
	if(item_disc_amt_2==null)item_disc_amt_2=0;
	if(disc_amount_ex==null)disc_amount_ex=0;
	if(m_qty==null)m_qty=0;
	if(m_unit==null)m_unit=unit;
	if(m_price==null)m_price=0;
	if(m_komisi==null)m_komisi=0;
	if(tenant==null)tenant=0;
	if(ref==null)ref=0;
	if(item_disc_prc_3==null)item_disc_prc_3=0;
	if(item_disc_amt_3==null)item_disc_amt_3=0;
	if(m_employee_id==null)m_employee_id="";
	if(m_employee_id!=""){
		$("#salesman").val(m_employee_id);
	}
...
function get_row(item_number){
	...
	$(".line-order").each(function(){
		...
		i++;
		if(td[C_CODE]==item_number){
			selected_row=i;
			selected_line_id=td[C_ID];
			retval=td;
		};
	});
	return retval;
}
function find_row(item_number){
	var i=0;
	var idx=-1;
	selected_line_id=0;
	selected_row=0;
...
//			item_total=qty*(retail.replace(/,/g,"")-item_disc_amt.replace(/,/g,"")-item_disc_amt_2.replace(/,/g,"")-item_disc_amt_3.replace(/,/g,""));
			item_total=(c_(qty)*c_(retail))-c_(item_disc_amt)-c_(item_disc_amt_2)-c_(item_disc_amt_3);
...
function find_nomor_so(){
	var nomor_so=$("#nomor_so").val();
	if(nomor_so==""){
		console.log("nomor sales order tidak diisi !");return false;
	}
	loading();
	$.ajax({
		type: "GET",
		url: CI_BASE+"index.php/sales_order/items/"+nomor_so+"/json",
		success: function(result){
			loading_close();
			var result = eval('('+result+')');
			...
			if(rows.length>0){
				for(i=0;i<rows.length;i++){
					item_number=rows[i].item_number;
					description=rows[i].description;
					qty=rows[i].quantity;
					unit=rows[i].unit;
					retail=rows[i].price;
					item_disc_prc=rows[i].discount;
					item_disc_amt=0;
					item_total=rows[i].amount;
				...
				log_err("Nomor sales order tidak ditemukan atau tidak ada item barangnya !");
			}
		},
		error: function(msg){
			loading_close();
