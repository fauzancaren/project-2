...
$(document).ready(function(){
		...
		$.ajax({type: "GET",url: url_cat+idx_cat_page,data:'',
			success: function(msg){	
				...
				for(i=0;i<data.length;i++){
					cat=cat+"<a href='#' onclick='list_barang("+data[i].kode+");return false' class='btn btn-app'><i class='fa fa-edit''></i> " + 
						data[i].category + "</a>";
				}
				$("#cat-content").html(cat);
			},
			error: function(msg){
				console.log(msg);}
		});			
	...
	function list_barang(cat) {
		...
		$.ajax({
			...
			success: function(msg){
				...
				var itm="";
				if(arBarang!=null){
					for(i=0;i<arBarang.length;i++){
						itm=itm+"<a href='#' onclick='alert("+arBarang[i].item_number+");return false' class='btn btn-app'><i class='fa fa-edit''></i> " + 
							arBarang[i].description.substring(1,10) + "<span class='badge'>Rp. "+arBarang[i].retail+"</span></a>";
					}
	...
	function save_nota() {
		...
		if (cek_credit_limit() && termin!="CASH"){
...
function clear_header_items(){
	$("#divNota").html("<table id='tbl_nota'  " +
		" cellpadding='5' class='nota-contentz table table-bordered'> " +
	    "  <tr><th>Qty</th><th>Nama Barang</th> " +
	    "  <th>Price</th><th>Jumlah</th> " +
	    " </tr> " +                        
	"</table>");
}
